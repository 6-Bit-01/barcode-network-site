...
New submitters can edit or replace their own mid/back-of-queue songs from the original browser using **Your songs**, while **Add another song** reuses existing intake. Editing permanently closes at the first three upcoming songs in a lane or within a conservative 10-minute music window. Next in Line, Now Playing and Wheel Chosen also lock editing; Wheel candidates pause edits during a spin and pending confirmation. Editing/uploading never reserves a song or delays the host. Submitters can change the song title, featured artists and private note to the host while keeping the current audio, or replace the link/MP3/WAV. The same cutoff applies to every edit. Slot counts, primary-artist identity and purchases are preserved. Saves are self-service and require no host editing or approval. See [replacement rules and deployment evidence](docs/queue-own-song-replacement-2026-09-24.md).
