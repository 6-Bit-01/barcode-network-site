...
New submitters can replace their own mid/back-of-queue songs from the original browser using **Your songs**, while **Add another song** reuses existing intake. Replacement permanently closes at the first three upcoming songs in a lane or within a conservative 10-minute music window. Next in Line, Now Playing and Wheel Chosen also lock replacement; Wheel candidates pause edits during a spin and pending confirmation. Editing/uploading never reserves a song or delays the host. Slot counts, artist credits and purchases are preserved. See [replacement rules and deployment evidence](docs/queue-own-song-replacement-2026-09-24.md).
