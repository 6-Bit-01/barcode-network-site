...
Submitter editing and replacement are **disabled by default pending private rehearsal**. They require `BARCODE_QUEUE_SUBMITTER_EDITING_ENABLED=true` on the server. This separate capability hides the editor and rejects edits/replacement upload authorization without changing the queue’s open/closed setting, new submissions, host controls or purchases. Leave it unset in production; isolated preview/test environments may explicitly enable it for rehearsal. When enabled, new submitters can edit or replace their own mid/back-of-queue songs from the original browser using **Your songs**, while **Add another song** reuses existing intake. Each track gets **one saved edit**, combining title, featured artists, private note and/or a replacement link/MP3/WAV. Only a committed change consumes it; unchanged saves, cancelled forms, rejected saves and unfinished uploads do not. The server preserves the allowance across tabs and reloads, and the song shows **Edit used** afterward. Editing permanently closes at the first three upcoming songs in a lane or within a conservative 10-minute music window. Next in Line, Now Playing and Wheel Chosen also lock editing; Wheel candidates pause edits during a spin and pending confirmation. Editing/uploading never reserves a song or delays the host. The same cutoff applies to every edit, including unused allowances. Slot counts, primary-artist identity and purchases are preserved. Saves are self-service and require no host editing or approval. See [replacement rules and deployment evidence](docs/queue-own-song-replacement-2026-09-24.md).

When enabled for the show, intake also offers **Free queue + Signal Hold** at the saved session price. It accepts the song before opening the existing owner-only checkout; protection still requires paid webhook confirmation and closes for the next two to play. Priority remains a separate purchase. Show Management hides new-session default prices while a current show exists, previews edits immediately, and applies them only after Save Session Options.

The Broadcast Deck is the live show companion and links to, but does not embed, the separate Broadcast Archive. Its full-show activity route retains public-safe submissions, playback movement and outcomes, removals and returns, Wheel unlocks and ceremony changes, Signal Hold movement, broadcast state, and sponsor-break transitions; visitors can filter and reorder the complete retained log without changing queue state. The queue’s separate “Done submitting—or just watching?” card links to the Deck once visitors have finished adding all their songs or are only watching; individual accepted-submission receipts do not send them away. Radio has no Deck links or mentions in any state. Its right-hand feature always presents the latest archived public show, including during a live broadcast. The left-hand card combines current submission actions with the next scheduled broadcast’s date, Pacific/local times and countdown when off-air. Its subtle past-broadcast link points right on desktop and down on phones to that Archive panel. Planned times never establish live or intake status. The Archive feature links onward to artists and BNL’s released music; HQ’s compact feature retains its existing live Deck behavior. See `docs/radio-archive-discovery.md` for this first item 7 presentation slice and the remaining discovery plan. `/queue` resolves straight to the current authorized session, with no gateway page; when none is active it returns to Radio’s queue status. The authorized session page preserves the original six-second digital BARCODE portal on first entry per browser/session, with Enter Now/Escape and reduced-motion handling. Checkout returns bypass the intro. The shared TikTok button opens the profile between broadcasts and the configured live URL only for a current real public broadcast; scheduled time and private tests alone do not activate it. Public queue panels, Archive names, the Deck welcome, mobile navigation and the submission animation adapt to narrow/short screens; see `docs/public-show-mobile-2026-09-23.md`. Radio’s primary queue card reuses the global queue read and links directly to the current public session, including closed-intake/full queues that remain viewable. It distinguishes checking, unavailable, open, closed, full, and live states, without advertising private sessions. The Broadcast Archive and sitemap do not advertise the Deck. Deck copy directs unfinished submissions back to the queue and frames the Deck as the watch-along for people who are done submitting or are only watching. A track may expose an `Open music` action only when it carries a validated external HTTP(S) link. Uploaded tracks may still appear as non-clickable show metadata, but their file URLs and file player never enter the Deck. Public Archive history begins on 2026-08-24 and is rebuilt only from archived sessions explicitly marked `live_broadcast`; active shows remain on the Deck, and older shows are not automatically imported. Rehearsals, simulations, private upload URLs, payment state, moderation data, private contact details, and browser tokens remain outside the public projection. Submitted TikTok handles are attribution for who submitted a track, not verified identity or artist ownership.
