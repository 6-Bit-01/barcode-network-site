...
`contractVersion: 1`, `kind: bnl_own_art`, `art`, `pngBase64`. Art contains only
`artId`, `title`, `meaning`, UTC `createdAt`, original PNG `sha256`, and `journal`
...
The route bounds the streamed body to 2,850,000 bytes and decoded PNG to 2 MiB,
...
returned. `GET /api/bnl/art?id=...` serves only currently public PNGs with
