...
The visual director treats show state as a set of naturally blended layers rather than hard scene swaps. Intake, queue, track, Wheel, sponsor, pressure, final, complete, and system states each gain a distinct procedural signature; idle periods receive occasional restrained ambient moments. Intake deliberately uses the full portrait canvas because the performer camera is not visible during submissions. Camera-visible phases feather ordinary effects away from the performer field while retaining moving edge detail and occasional shallow inward bleed. The Wheel vortex occupies an animated outer band around the Wheel content, with only faint interior light so names remain readable. Track starts/skips, Priority sends/confirmations, Wheel gains/launches/spins, sponsor changes, intake changes, show start/completion, and stage shifts receive bounded automatic accents. A permanent receiver can infer Wheel-gained and other state-edge moments from consecutive display-safe snapshots, while authoritative timestamped events are projected directly when the existing queue owner records them. Every event occurrence and every manual-cue nonce selects a new deterministic composition, so repeated cues vary in position, direction, particle layout, timing, and geometry while remaining synchronized across a single occurrence. Palette, state, complete seeded compositions, cue envelopes, event envelopes, and music-response channels ease independently to avoid abrupt visual cuts.

The Level 1 music-response path uses the authoritative player clock when a provider exposes one. Each track occurrence selects one of ten deterministic visual families—edge spectrum, oscilloscope ribbons, tape feedback, Matrix rain, ASCII terminal, pixel-sort storm, lightning switchyard, laser lattice, particle pressure, and signal constellation—plus one of six repeatable rhythm personalities for tempo, density, swing, and frequency balance. Waveforms and meter geometry are confined to the music dispatcher, while the remaining music families deliberately use different motion languages and silhouettes. The local visual clock drives beat, accent, low-, mid-, and high-frequency proxy channels between player heartbeats. External players with no usable playback or audio signal automatically receive that synthetic music response at active-track intensity, with no host prompt or control. The private same-origin MP3/WAV player additionally uses a Web Audio analyser and attaches bounded aggregate energy, bass, mid, treble, and peak values to its existing 1 Hz player-sync heartbeat. The public visual projection never receives audio samples, an upload URL, track identity, or source identifier, and the automatic path adds no Redis command or polling path. These reads and effects do not mutate queue, Wheel, skip, Priority, payment, or playback state.
