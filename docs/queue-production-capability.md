...
The visual director treats show state as naturally blended layers rather than hard scene swaps. Intake, queue, track, Wheel, pressure, final, complete, and system states each gain a distinct procedural signature; intake uses the full portrait canvas because the performer is not visible during submissions. Camera-visible phases retain 20% of ordinary effect energy at the center through a softer feathered field while preserving active edge detail. A separate tightly bounded intrusion pass permits only sparse scan lines, the lightning-switchyard trace, the Lightning cue, and the Signal Breach scan to regain controlled center presence; dense glyph, meter, particle, and block fields remain attenuated. Opening a queue session exposes the pre-show source; the authoritative transition into `broadcast_active` emits the show-start event, with an intake-to-live edge retained only for legacy snapshots that carry no broadcast phase. That event renders an unmistakable split-edge/top/bottom ignition outside ordinary performer attenuation while keeping the central performer corridor clear.

The Wheel effect is a continuous hollow portal rather than a stack of dotted rings: a dark violet/black depth throat, broad spiral sheets, warped caustic ribbons, curved suction streaks, rim lightning, and an edge-to-edge outer bloom rotate from the same accumulated phase. One candidate already receives four ribbons, at least 24 streaks, and rim lightning. Density and translucent inward reach rise continuously with visible candidate count, and fourteen candidates enters an unmistakable storm tier with two additional spiral sheets, at least sixteen additional suction streaks, and at least five lightning arcs; very large Wheels remain at a bounded overdrive profile instead of losing geometry or overwhelming the renderer. Only the translucent sheets may reach farther toward the labels as counts rise; opaque depth and hard caustics stay in the candidate-calibrated outer zone, and the center remains faint enough for names to stay readable. Ready, spinning, re-encrypting, result, and confirmed states share one ceremony-stable portal personality and accumulated phase, changing continuous velocity and restrained energy without a staged choreography or angular jumps. Track start/skip, Priority sent/confirmed, Wheel gained/launch/spin, intake changes, show completion, and stage shifts keep their bounded accents; Wheel accents only surge the main portal rim instead of drawing a second independently phased portal. Authoritative timestamped events take precedence, while the permanent receiver infers only missing display-safe state edges. Automatic state-edge events and manual-cue nonces select deterministic but varied compositions, and palette, state, cue, event, and music-response envelopes ease independently to avoid abrupt cuts.

The Level 1 music-response path uses the authoritative player clock when available. Each track occurrence selects one of ten deterministic visual families—edge spectrum, oscilloscope ribbons, tape feedback, Matrix rain, ASCII terminal, pixel-sort storm, lightning switchyard, laser lattice, particle pressure, and signal constellation—plus a repeatable rhythm personality. Waveform and meter geometry remain confined to their two music families; the other eight retain distinct motion languages and silhouettes. Every family now composes four explicit audio layers: sustained bass reveals mass, depth, anchors, pressure, or charged foundations; sustained mids reveal structure, topology, routing, panes, or connective geometry; treble reveals fine fragments, sparks, cursors, tears, shutters, or comets; and a strict all-band tapestry layer appears only when bass, mids, and treble are present together. Short-lived band-specific onsets add impact without leaking a kick into unrelated bands. A single quiet-knee transfer suppresses low-level helper noise and preserves accelerating headroom at loud levels, while fast attack and graduated per-band release keep real arrivals legible. Build memory follows actual broadband and band fullness with a fast attack and gradual release; four-bar phrase position and track progress—or a deterministic multi-minute fallback anchored to the current track occurrence when duration is unavailable—may evolve layouts but cannot manufacture density during silence. Scenes therefore add and remove counts, topology, depth, density, and secondary layers rather than only changing opacity or speed. The common between-song transmission bed recedes as track activity rises so its bars and tracking lines do not make the ten families look alike, while each selected scene retains a restrained identity floor and expands continuously with actual audio. External players with no usable playback or audio signal automatically receive the six-profile synthetic response, with no host prompt or control. The private same-origin MP3/WAV player may additionally publish bounded aggregate energy and band values through its existing 1 Hz player-sync heartbeat. The public visual projection still receives no samples, media URL, track identity, or source identifier, adds no Redis polling path, and cannot mutate queue, Wheel, skip, Priority, payment, or playback state.

The optional BARCODE Audio Bridge adds direct show-computer response without changing that server contract. It is a current-user Windows tray helper that captures only the default Speakers render endpoint through WASAPI loopback, reduces that signal locally to bounded energy, bass, mid, treble, peak, beat, tempo, and confidence values, and serves those values only from `127.0.0.1:43120`. The permanent Show Visuals Link polls that loopback endpoint automatically while an active queue session exists; the helper starts Speakers capture on those requests and stops it a few seconds after they stop. Chrome and Windows Media Player therefore drive the visuals when they play through the default Speakers device, while the separately sourced microphone is excluded unless Windows itself is deliberately configured to monitor it into Speakers. No audio samples or analysis frames are uploaded, and the bridge creates no Redis or Vercel traffic. An active, warmed bridge remains authoritative through true silence so zero input produces restrained zero audio layers instead of snapping to synthetic music; only a missing, stale, blocked, restarting, inactive, or not-yet-warmed bridge leaves the six-profile synthetic response in control. There is no operator capture button.
