...
The separate Radio Visuals browser source lives at the permanent `/overlay/radio-visuals` route. It reuses the queue, live-scene, and player-sync owners and exposes only a display-safe projection: show stage and aggregate counts, safe artist/title/source type, live scene copy, and fresh play/pause/timeline state. It does not expose session identifiers, queue-entry identifiers, source or upload URLs, handles, contact fields, notes, legal records, payment fields, or player provider IDs. The receiver keeps a low-frequency animated standby poll so a browser source already placed in TikTok Studio can detect a new show without a new URL, then switches to the live polling interval while that show is active. Level 1 derives procedural motion from show progress, track identity, and the existing player timeline; `audioEnergy` remains explicitly null until a separate first-party audio-analysis bridge exists.
