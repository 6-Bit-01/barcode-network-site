...
The existing Wheel ceremony also has a permanent, isolated browser source at `/overlay/wheel`. It uses the existing live-overlay wheel state, queue resolver, candidate order, winner, animation phases, and audio files without changing queue or spin mechanics. The read-only wheel endpoint returns no scene while the Wheel is inactive, so the `1080×1080` source remains entirely `#FF5A00` and keys out until the existing Launch Wheel control activates it. The permanent receiver polls every 15 seconds on standby, every 2 seconds during an active show without a ceremony, and every second while the Wheel is visible. TikTok Studio must keep that source active with sound enabled for ceremony audio.
