...
After review, merge the site stack in order (423, 424, 425, then this PR), retarget/rebase as needed and reverify the final merged candidate. Let the existing Vercel main deployment finish; record its Ready status and served commit. Commercial Player 1.0.25 remains a dependency inherited from PR 425. Refresh the queue/admin/archive tabs so rule version 1.1 and current scripts agree. Do not restore any older production data to deploy or undo this source change.
