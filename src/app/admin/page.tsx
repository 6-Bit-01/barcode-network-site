...
export default function AdminPage() {
  const { isLive, toggleLive, streamUrl, setStreamUrl, isScheduled, sponsorsActive, setSponsorsActiveSignal, manualOverride, lastError, persisted } = useLiveStatus();
...
function AdminContent({ isLive, toggleLive, streamUrl, setStreamUrl, isScheduled, sponsorsActive, setSponsorsActiveSignal, manualOverride, urlInput, setUrlInput, lastError, persisted }: any) {
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><div className="border border-border bg-surface p-6"><h2 className="text-[10px] uppercase tracking-[0.5em] text-muted mb-6">BARCODE Radio — Live Status</h2><button onClick={toggleLive} className="w-full px-4 py-3 text-sm uppercase tracking-widest border border-accent text-accent hover:bg-accent hover:text-background transition-all font-bold">{isLive ? 'GO OFFLINE':'GO LIVE'}</button><div className="text-xs text-muted/50 mt-3"><p>// Scheduled: {isScheduled ? 'YES' : 'NO'}</p><p>// Override: {manualOverride ? 'ACTIVE' : 'NONE'}</p><p>// Persistence: {persisted === null ? 'UNKNOWN' : persisted ? 'REDIS' : 'IN-MEMORY'}</p>{lastError && <p className='text-danger'>{lastError}</p>}</div></div><div className="border border-border bg-surface p-6"><h2 className="text-xs sm:text-sm uppercase tracking-[0.5em] text-muted mb-6">Stream URL</h2><input type="url" value={urlInput} onChange={(e) => setUrlInput(e.target.value)} className="w-full bg-background border border-border px-3 py-2.5 text-sm" /><button onClick={() => setStreamUrl(urlInput)} className="mt-4 w-full px-4 py-2.5 text-sm uppercase tracking-widest border border-border text-muted hover:border-accent hover:text-accent transition-all">Update Stream URL</button></div><div className="border border-accent/40 bg-surface p-6 shadow-[0_0_28px_rgba(255,0,0,0.08)]"><h2 className="text-[10px] uppercase tracking-[0.5em] text-accent mb-6">Sponsor Signal</h2><div className="grid grid-cols-2 gap-3"><button onClick={() => setSponsorsActiveSignal(true)} className={`px-4 py-3 text-xs uppercase tracking-widest border transition-all font-bold ${sponsorsActive ? 'border-accent bg-accent text-background shadow-[0_0_18px_rgba(255,0,0,0.25)]' : 'border-border text-muted hover:border-accent hover:text-accent'}`}>Activate</button><button onClick={() => setSponsorsActiveSignal(false)} className={`px-4 py-3 text-xs uppercase tracking-widest border transition-all font-bold ${!sponsorsActive ? 'border-border bg-background/60 text-foreground' : 'border-danger/40 text-danger hover:bg-danger hover:text-background'}`}>Clear</button></div><div className="text-xs text-muted/50 mt-3"><p>// Sponsors Active: {sponsorsActive ? 'YES' : 'NO'}</p><p>// Public banner: {sponsorsActive ? 'VISIBLE' : 'HIDDEN'}</p><p>// Use this while sponsor reads or ads are running.</p></div></div></div>
