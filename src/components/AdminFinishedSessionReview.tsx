    ...
    <div className="space-y-6">
      ...
      <section className="border border-border bg-surface p-5 space-y-4">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="border border-border bg-background/40 p-4 space-y-3 text-sm lg:col-span-3">
            <p className="text-xs uppercase tracking-widest text-muted">Wheel</p>
            <div className="flex flex-wrap gap-x-6 gap-y-2"><span>Launches: <strong>{report.operations.wheel.launches}</strong></span><span>Spins: <strong>{report.operations.wheel.spins}</strong></span><span>Re-encryptions: <strong>{report.operations.wheel.reencryptions}</strong></span><span>Rejected results: <strong>{report.operations.wheel.rejectedResults}</strong></span><span>Confirmed: <strong>{report.operations.wheel.confirmations}</strong></span><span>Cancelled: <strong>{report.operations.wheel.cancellations}</strong></span><span>Ceremony time: <strong>{duration(report.operations.wheel.ceremonySeconds)}</strong></span><span>Average ceremony: <strong>{duration(report.operations.wheel.averageCeremonySeconds)}</strong></span></div>
            {report.operations.wheel.spinTimings.length > 0 && <div className="border-t border-border/60 pt-3 space-y-1 text-xs">
              <p className="uppercase tracking-widest text-muted">Spin timing</p>
              {report.operations.wheel.spinTimings.map((spin, index) => <p key={`${spin.startedAt}-${index}`}><strong>Spin {index + 1}:</strong> {timestamp(spin.startedAt)} → {timestamp(spin.endedAt)} · {duration(spin.durationSeconds)}</p>)}
            </div>}
          </div>
        </div>
      </section>
