...
export function AdminRadioQueueControl() {
  ...
  return (
    <div className={`${playerPadding} space-y-6`}>
      ...
      {canControlSession && <section className="grid gap-3 border border-border bg-surface p-4 text-xs md:grid-cols-5"><div><p className="uppercase tracking-widest text-muted">Opening state</p><p className="mt-1 font-bold text-foreground">{openingState ? "Yes" : "No"}</p></div><div><p className="uppercase tracking-widest text-muted">Next owed</p><p className="mt-1 font-bold text-foreground">{state?.nextNonPriorityLane === "regular" ? "Free" : "Wheel"}</p></div><div><p className="uppercase tracking-widest text-muted">Interrupted Next</p><p className="mt-1 font-bold text-foreground">{displacedNonPriorityNext ? submittedTitle(displacedNonPriorityNext) : "None"}</p></div><div><p className="uppercase tracking-widest text-muted">Active Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{activePriorityCount}</p></div><div><p className="uppercase tracking-widest text-muted">Held Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{heldPriorityCount}</p></div></section>}
