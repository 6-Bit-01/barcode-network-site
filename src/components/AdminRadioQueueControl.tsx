...
type AdminQueueAction = "pullNext" | "startShow" | "addWheelSpinOwed" | "load" | "finish" | "remove" | "priority" | "regular" | "wheel" | "moveBack" | "spotlight" | "removeSpotlight" | "restoreRegular" | "restorePriority" | "pausePriority" | "resumePriority" | "stageFirstFree";
...
export function AdminRadioQueueControl() {
  ...
  async function action(id: string, next: AdminQueueAction) { await post(next === "pullNext" || next === "stageFirstFree" || next === "startShow" || next === "addWheelSpinOwed" ? { action: next } : { id, action: next }); }
  ...
  const openingState = state?.session?.playbackStarted !== true;
  ...
  return (
    <div className={`${playerPadding} space-y-6`}>
      ...
      {canControlSession && <section className="grid gap-3 border border-border bg-surface p-4 text-xs md:grid-cols-7"><div><p className="uppercase tracking-widest text-muted">Opening state</p><p className="mt-1 font-bold text-foreground">{openingState ? "Yes" : "No"}</p></div><div><p className="uppercase tracking-widest text-muted">Next owed</p><p className="mt-1 font-bold text-foreground">{state?.nextNonPriorityLane === "regular" ? "Free" : "Wheel"}</p></div><div><p className="uppercase tracking-widest text-muted">Interrupted Next</p><p className="mt-1 font-bold text-foreground">{displacedNonPriorityNext ? submittedTitle(displacedNonPriorityNext) : "None"}</p></div><div><p className="uppercase tracking-widest text-muted">Show Started</p><p className="mt-1 font-bold text-foreground">{state?.session?.showStarted ? "Yes" : "No"}</p></div><div><p className="uppercase tracking-widest text-muted">Wheel Owed</p><p className="mt-1 font-bold text-cyan-200">{state?.session?.wheelSpinsOwed ?? 0}</p></div><div><p className="uppercase tracking-widest text-muted">Active Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{activePriorityCount}</p></div><div><p className="uppercase tracking-widest text-muted">Held Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{heldPriorityCount}</p></div></section>}
