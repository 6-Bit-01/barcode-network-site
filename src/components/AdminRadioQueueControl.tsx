...
export function AdminRadioQueueControl() {
  ...
  return (
    <div className={`${playerPadding} space-y-6`}>
      ...
      {canControlSession && <section className="grid gap-4 xl:grid-cols-2">
        <section className="border border-[#ffaa00]/50 bg-[#ffaa00]/5 p-4 space-y-2">
          <p className="text-xs uppercase tracking-[0.32em] text-[#ffaa00]">Priority / Payment</p>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3 text-xs">
            <div className="border border-[#ffaa00]/40 bg-background/40 p-2"><p className="uppercase tracking-widest text-muted">Payments</p><p className="mt-1 font-bold text-foreground">{state?.session?.priorityUpgradePaymentsEnabled ? "Enabled" : "Disabled"}</p></div>
            <div className="border border-[#ffaa00]/40 bg-background/40 p-2"><p className="uppercase tracking-widest text-muted">Price</p><p className="mt-1 font-bold text-foreground">{formatPrice(state?.session?.priorityUpgradePriceCents ?? 0, state?.session?.priorityUpgradeCurrency ?? "usd")}</p></div>
            <div className="border border-[#ffaa00]/40 bg-background/40 p-2"><p className="uppercase tracking-widest text-muted">Payment Processing</p><p className="mt-1 font-bold text-[#ffaa00]">{(state?.queue ?? []).filter((entry) => ["checkout_pending","requested","paid_needs_attention"].includes(entry.priorityUpgradeStatus ?? "none")).length}</p></div>
            <div className="border border-[#ffaa00]/40 bg-background/40 p-2"><p className="uppercase tracking-widest text-muted">Active Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{activePriorityCount}</p></div>
            <div className="border border-[#ffaa00]/40 bg-background/40 p-2"><p className="uppercase tracking-widest text-muted">Held Priority</p><p className="mt-1 font-bold text-[#ffaa00]">{heldPriorityCount}</p></div>
          </div>
        </section>
        <section className="border border-border bg-surface p-4">
          <p className="text-xs uppercase tracking-[0.32em] text-accent">Overlay / Sponsor / Visual Control</p>
