...
function StartNewSession({ locked, queueIsOpen, onCloseSubmissions, onEnd, title, description, purpose, bnlPublicationStatus, trackLimitPerArtist, queueCapacity, submissionCooldownSeconds, priorityUpgradesEnabled, priorityUpgradePriceCents, priorityUpgradeCurrency, priorityStartError, signalHoldEnabled, signalHoldPriceCents, signalHoldCurrency, signalHoldStartError, onTitle, onDescription, onPurpose, onBnlPublicationStatus, onTrackLimit, onCapacity, onSubmissionCooldown, onPriorityEnabled, onPriorityPrice, onSignalHoldEnabled, onSignalHoldPrice, onStart, sessionId }: StartNewSessionProps) {
  ...
  return (
    <section className={`space-y-5 border p-6 ${locked ? "border-danger/60 bg-danger/10" : "border-accent/40 bg-surface"}`}>
      ...
      {!locked && <><div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="space-y-4 border border-cyan-300/35 bg-cyan-300/5 p-4 lg:col-span-2">
          <div><p className="text-xs uppercase tracking-[0.3em] text-cyan-200">Show type and BNL access</p><p className="mt-1 text-sm text-muted">Choose whether BNL gets no queue data, private read-only queue data, or public read-only queue data. Payment and sensitive fields are always excluded.</p></div>
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <label className="space-y-2"><span className="text-xs uppercase tracking-widest text-muted">BNL queue access</span><select disabled={locked} value={bnlPublicationStatus === "recap_approved" ? "public_copy_approved" : bnlPublicationStatus} onChange={(event) => onBnlPublicationStatus(event.target.value as QueueSessionBnlPublicationStatus)} className="w-full border border-border bg-background px-3 py-2.5 text-sm disabled:opacity-50"><option value="private">No BNL queue access</option><option value="runtime_only">Private BNL queue access</option>{purpose === "live_broadcast" && <option value="public_copy_approved">Public BNL queue access</option>}</select></label>
          </div>
          <p className="text-xs text-muted">Tests can use private BNL access without appearing in public BNL outputs or the public Broadcast Archive. Public access is offered only for live broadcasts. BNL cannot change the queue, and no access level enables automatic memory or dossiers.</p>
        </section>
        ...
        <section className="space-y-4 border border-cyan-300/40 bg-cyan-300/5 p-4 lg:col-span-2">
          <div><p className="text-xs uppercase tracking-[0.3em] text-cyan-200">Signal Hold</p><p className="mt-1 text-sm text-muted">Set the price and enable Signal Hold for this show.</p></div>
...
function CurrentSession({ session, onPost, onEnd }: { session: QueueSessionSummary | null | undefined; onPost: (body: Record<string, unknown>) => Promise<QueueState | null>; onEnd: () => void }) {
  ...
  useEffect(() => {
    if (!session || priorityEditing) return;
    ...
    <section className="border border-border bg-surface p-5 space-y-4">
      ...
      {priorityEditing ? (
        <section className={`space-y-5 border bg-accent/5 p-5 transition-all duration-300 ${priorityJustSaved ? "border-accent shadow-[0_0_28px_rgba(255,0,0,0.28)]" : "border-accent/40"}`}>
          ...
          <fieldset disabled={prioritySaving} className="grid min-w-0 gap-4 lg:grid-cols-2 disabled:opacity-60">
          ...
          <p className="text-xs text-muted">Prices above preview your edits. They take effect after Save Session Options.</p>
