    ...
    <section className={`space-y-5 border p-6 ${locked ? "border-danger/60 bg-danger/10" : "border-accent/40 bg-surface"}`}>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <section className="space-y-4 border border-cyan-300/35 bg-cyan-300/5 p-4 lg:col-span-2">
          <div><p className="text-xs uppercase tracking-[0.3em] text-cyan-200">Show type and BNL access</p><p className="mt-1 text-sm text-muted">Choose what kind of session this is. Tests stay private. For a live broadcast, choose exactly how much sanitized queue information BNL may use.</p></div>
          <div className="grid gap-3 lg:grid-cols-2">
            ...
            <label className="space-y-2"><span className="text-xs uppercase tracking-widest text-muted">What can BNL use from this show?</span><select disabled={locked || purpose !== "live_broadcast"} value={bnlPublicationStatus} onChange={(event) => onBnlPublicationStatus(event.target.value as QueueSessionBnlPublicationStatus)} className="w-full border border-border bg-background px-3 py-2.5 text-sm disabled:opacity-50"><option value="private">BNL gets nothing from this show</option><option value="runtime_only">BNL can see the live queue during this show only</option><option value="recap_approved">BNL can also use finished tracks for a show recap</option><option value="public_copy_approved">BNL can also use sanitized show facts in public messages</option></select></label>
          </div>
          <p className="text-xs text-muted">Tests automatically use “BNL gets nothing.” These choices do not enable BNL memory or dossiers and do not change queue behavior.</p>
        </section>
