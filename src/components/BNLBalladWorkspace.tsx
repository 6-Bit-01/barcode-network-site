...
export function BNLBalladWorkspace({ initialShowId = "" }: { initialShowId?: string }) {
  ...
  const publishTasks = balladPublishTasks({ pending, dirtySections, hasVersion: Boolean(workingVersion), pendingUpload: Boolean(pendingUpload), hasWorkingTakes: Boolean(doc?.audio.some(a => a.versionId === versionChoice)), hasConfirmedRecording: Boolean(releaseTake && releaseVersion) });
  const nextTask = publishTasks[0];
  function goToStep(targetStage: BalladStage, target: string) {
  ...
  async function completeTask(task: BalladPublishTask) {
    goToStep(task.stage, task.target);
    const saves: Record<string, [string, Record<string, unknown>]> = {
    ...
    if (task.id === "draft" && (!title.trim() || !lyrics.trim())) {
      setError("Add a title and lyrics in Song, then save your edits."); return;
    }
    if (saves[task.id]) {
      const [action, payload] = saves[task.id];
      if (await act(action, payload)) clearDirty(task.id);
    } else if (task.id === "upload" && pendingUpload) {
      if (await act("attachAudio", pendingUpload)) setPendingUpload(null);
    }
  }
  ...
  return <fieldset disabled={busy || pending} className="space-y-6" aria-label="BNL Broadcast Ballads workspace">
    ...
    {!snapshot ? <p className="text-muted">{error ? "Use Reload workspace to try again." : "Loading the recording desk…"}</p> : <>
      ...
      <details id="ballad-automation" tabIndex={-1} className="rounded border border-border p-4"><summary className="cursor-pointer text-sm font-bold text-foreground">After-show automation · {snapshot.config.enabled ? "On" : "Off"}</summary>
      ...
      {doc && snapshot.show ? <>
        ...
        <section id="ballad-writing" tabIndex={-1} className="space-y-4 rounded border border-border p-5">
        ...
        <div role="status" aria-live="polite" className="flex flex-wrap items-center justify-between gap-4 rounded border border-accent/50 bg-accent/5 p-4">
          <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-widest text-accent">{nextTask ? "Next step" : "Ready for release"}</p><h3 className="mt-1 text-lg font-bold text-foreground">{nextTask?.title ?? "Review and publish your song"}</h3><p className="mt-1 text-sm text-muted">{nextTask?.detail ?? "The recording and saved details are ready. Review the exact release in Publish."}</p>{publishTasks.length > 1 && <p className="mt-2 text-xs text-muted">{publishTasks.length} steps left before publishing</p>}</div>
          {nextTask?.action ? <button type="button" className={primaryClass} onClick={() => completeTask(nextTask)}>{nextTask.action} →</button> : !nextTask && <button type="button" className={primaryClass} onClick={() => goToStep("Publish", "ballad-release")}>Review release →</button>}
        </div>
        <section id="ballad-song" tabIndex={-1} hidden={stage !== "Song"} className="space-y-4 rounded border border-border p-5">
        ...
        <section id="ballad-recording" tabIndex={-1} hidden={stage !== "Recording"} className="space-y-4 rounded border border-border p-5"><h3 className="text-lg font-bold text-foreground">Upload & choose audio</h3><p className="text-sm text-muted">Create and select your song in Suno, then bring the MP3 or WAV here. Alternate takes stay available.</p>
        ...
        <section id="ballad-release" tabIndex={-1} hidden={stage !== "Publish"} className="space-y-4 rounded border border-border p-5"><h3 className="text-lg font-bold text-foreground">Release details</h3>
          ...
          <div className="flex flex-wrap gap-2"><button type="button" disabled={busy} className={buttonClass} onClick={async () => { if (await act("savePresentation", { presentation })) clearDirty("presentation"); }}>Save release details</button><button type="button" aria-describedby="ballad-publish-requirements" disabled={busy || publishTasks.length > 0} className={primaryClass} onClick={() => { if (window.confirm("Publish the confirmed take, its saved lyrics and the saved release details?")) void act("publish"); }}>{doc.published ? "Publish updated song" : "Publish this song"}</button></div>
        ...
        <div className="sticky bottom-3 z-10 flex flex-wrap items-center justify-between gap-3 rounded border border-accent/40 bg-background p-4 shadow-xl"><span className="text-sm text-foreground">{nextTask ? `Next: ${nextTask.title}` : "Ready to publish · All changes saved"}</span>{nextTask?.action ? <button type="button" className={primaryClass} onClick={() => completeTask(nextTask)}>{nextTask.action} →</button> : !nextTask && <button type="button" className={primaryClass} onClick={() => goToStep("Publish", "ballad-release")}>Review release →</button>}</div>
