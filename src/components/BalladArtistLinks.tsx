...
export function BalladLinkedText({ text, artistLinks }: { text: string; artistLinks?: BalladArtistLink[] }) {
  return <>{balladLinkedTextParts(text, artistLinks).map((part, i) => part.link ? <Link key={i} href={broadcastArchiveArtistHref(part.link.projectKey)} className="text-accent underline underline-offset-4">{part.text}</Link> : part.text)}</>;
}
...
export function BalladArtistCards({ artistLinks }: { artistLinks?: BalladArtistLink[] }) {
  ...
  return <section className="border-t border-border p-6" aria-label="Linked artist cards">
    ...
    <div className="mt-3 flex flex-wrap gap-3">{profiles.map(profile => <Link key={profile.projectKey} href={broadcastArchiveArtistHref(profile.projectKey)} className="max-w-full rounded border border-accent/40 px-4 py-3 text-sm text-accent hover:border-accent">
