...
export function BalladPlayback({ ballad }: { ballad: PublicBallad }) {
  ...
  const added = audio.playlist.some(item => item.key === track.key);
  return <div className="flex flex-wrap items-center gap-3">
    ...
    <button type="button" onClick={() => audio.controller.add(track)} aria-label={`${added ? "In playlist:" : "Add to playlist:"} ${track.title}`} className="inline-flex min-h-11 items-center rounded border border-accent/40 px-4 py-2 text-sm font-bold text-accent hover:bg-accent/10">{added ? "✓ In playlist" : "+ Playlist"}</button>
