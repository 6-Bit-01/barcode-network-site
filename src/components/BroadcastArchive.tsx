...
function artistSearchText(artist: QueuePublicProjectHistory): string {
  return [artist.projectLabel, ...(artist.aliases ?? []), ...artist.tracks.map(searchableTrack)].join(" ").toLocaleLowerCase("en-US");
}
    ...
    <article className="border-t-2 border-border/80 py-4 first:border-t-0 first:pt-0">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          ...
          {track.collaboratorNames && <p className="mt-1 text-xs text-muted"><span className="uppercase tracking-widest text-accent/80">Featuring</span> {track.featuredArtists?.length ? track.featuredArtists.map((credit, index) => <span key={index}>{index > 0 && ", "}<Link className="underline underline-offset-4 hover:text-accent" href={archiveSelectionHref(archiveBaseHref, "artists", credit.projectKey ?? `featured:${normalizeBroadcastArchiveProjectKey(credit.name)}`)}>{credit.name}</Link></span>) : track.collaboratorNames}</p>}
...
function ShowDetail({ show, archiveBaseHref, ballad }: { show: QueuePublicShowStats; archiveBaseHref: string; ballad?: PublicBallad }) {
  ...
  return (
    <section id="show-card" aria-labelledby="selected-show-heading" className="order-first scroll-mt-24 lg:order-last border border-border bg-surface p-5 sm:p-6">
      ...
      {ballad && <BroadcastBallad key={ballad.show.sessionId} ballad={ballad} />}
  ...
  return (
    <section id="artist-card" aria-labelledby="selected-artist-heading" className="order-first scroll-mt-24 lg:order-last border border-border bg-surface p-5 sm:p-6">
      ...
      <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5">
        <Stat label="Primary submissions" value={artist.acceptedSubmissionCount ?? artist.submittedTrackCount} detail="Accepted primary credits establish this artist card." />
      ...
      <div className="mt-6">
        ...
        {!artist.tracks.length && <p className="mt-4 text-sm text-muted">This artist has submitted music. No broadcast playback has been recorded in the Archive yet.</p>}
...
export function BroadcastArchive({
  initialStats,
  initialBallads = [],
  initialBalladsUnavailable = false,
  initialView = "shows",
  initialShowId = "",
  initialArtistKey = "",
  ...
  initialBallads?: PublicBallad[];
  initialBalladsUnavailable?: boolean;
    ...
    <div className="space-y-6">
      ...
      <section className="overflow-hidden border border-accent/45 bg-surface">
        ...
        <div className="grid grid-cols-2 gap-px bg-border sm:grid-cols-4">
          ...
          <Stat label="Artist cards" value={stats.artists.length} detail="Primary artists from accepted public submissions." />
      ...
      {view === "shows" ? (
        <div className="grid gap-5 lg:grid-cols-[minmax(16rem,0.34fr)_minmax(0,1fr)]">
          ...
          {selectedShow ? <ShowDetail show={selectedShow} archiveBaseHref={archiveBaseHref} ballad={previewMode ? undefined : ballads.find(ballad => ballad.show.sessionId === selectedShow.sessionId)} /> : <EmptyArchive kind="show" previewMode={previewMode} />}
        </div>
      ) : (
        <div className="grid gap-5 lg:grid-cols-[minmax(16rem,0.34fr)_minmax(0,1fr)]">
          <aside className="border border-border bg-surface p-4 lg:sticky lg:top-20 lg:self-start">
