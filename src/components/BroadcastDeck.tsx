...
function projectLink(track: QueuePublicTrack): string {
  return broadcastArchiveArtistHref(track.submittedArtistName);
}

function LiveTrackCard({ label, track, tone }: { label: string; track: QueuePublicTrack | null | undefined; tone: "amber" | "red" | "cyan" }) {
  ...
  return (
    <article className={`border bg-background/60 p-5 ${toneClass}`}>
      ...
      {track ? <>
        <Link href={projectLink(track)} className="mt-4 block text-xl font-black text-foreground hover:text-accent">{track.submittedArtistName}</Link>
...
export function BroadcastDeck() {
  ...
  async function load() {
    ...
    const queueUrl = submitterToken ? `/api/queue?submitterToken=${encodeURIComponent(submitterToken)}` : "/api/queue";
    try {
  ...
  useEffect(() => {
    setOrientationOpen(window.localStorage.getItem(ORIENTATION_STORAGE_KEY) !== "1");
  ...
  const queueHref = snapshot?.session && snapshot.session.status !== "archived" ? `/queue/${encodeURIComponent(snapshot.session.sessionId)}` : "/queue";
    ...
    <div className="space-y-6">
        ...
        <div className="relative border-b border-[#ffaa00]/20 bg-[linear-gradient(110deg,rgba(255,170,0,0.12),transparent_52%)] p-5 sm:p-7">
          ...
          <div className="relative flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-5 text-xs font-bold uppercase tracking-[0.38em] text-[#ffaa00]">Live show companion</p>
              <h1 className="mt-3 text-4xl font-black tracking-tight text-foreground sm:text-6xl">The Broadcast Deck</h1>
              <p className="mt-4 max-w-2xl text-sm leading-relaxed text-muted sm:text-base">Keep the show in your hand: follow Now Playing, what is coming up, the queue feed, Wheel movement, show progress, and your submissions from this browser.</p>
            </div>
            <div className="grid gap-2 sm:grid-cols-2 lg:w-[22rem] lg:grid-cols-1">
              ...
              <Link href="/radio/archive" className="border border-cyan-200/55 px-4 py-3 text-center text-xs font-black uppercase tracking-widest text-cyan-200 hover:bg-cyan-200 hover:text-background">Broadcast Archive</Link>
      ...
      {loaded && !isLive ? (
        <section className="border border-border bg-surface p-6 sm:p-8">
          ...
          <div className="mt-5 flex flex-wrap gap-2"><Link href="/radio/archive" className="border border-cyan-200 bg-cyan-200 px-4 py-3 text-xs font-black uppercase tracking-widest text-background">Open Broadcast Archive</Link><Link href="/queue" className="border border-border px-4 py-3 text-xs uppercase tracking-widest text-muted hover:border-accent hover:text-accent">Check queue gateway</Link></div>
        </section>
      ) : null}

      {isLive && <>
        <section className="grid gap-4 lg:grid-cols-2">
          <LiveTrackCard label="Now Playing" track={snapshot?.nowPlaying} tone="amber" />
          <LiveTrackCard label="Next In Line" track={snapshot?.upNext} tone="red" />
        </section>
