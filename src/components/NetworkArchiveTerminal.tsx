...
const bnlButtons = [{ label: "STATUS", command: "STATUS" }, { label: "TRACE", command: "BNL-01" }, { label: "RELAYS", command: "RELAYS" }, { label: "JOURNAL", command: "BNL LOG" }, { label: "HUB", command: "BNL HUB" }, { label: "MUSIC", command: "BNL MUSIC" }];
...
export function NetworkArchiveTerminal({ archive, restored, onLock }: { archive: ArchivePayload; restored: boolean; onLock: () => void }) {
  ...
  function execute(raw: string) {
    ...
    if (["BNL MUSIC", "BNL DISCOGRAPHY", "BNL BALLADS", "BALLADS"].includes(command)) return push(raw, <Block title="BNL-01 DISCOGRAPHY"><Link href="/bnl/music" className="text-accent underline">Open BNL’s discography →</Link></Block>);
