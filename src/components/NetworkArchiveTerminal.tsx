...
function Status({ data, loading, dossier }: { data: import("@/components/bnl-status").BNLStatus; loading: boolean; dossier?: ArchiveDossier }) { return <Block title="BNL-01 // LIVE PUBLIC RELAY"><dl className="grid gap-2 sm:grid-cols-2"><KV k="status" v={loading ? "SYNCING" : data.status} /><KV k="mode" v={data.mode ?? "UNKNOWN"} /><KV k="message" v={data.message ?? "No public message."} /><KV k="directive" v={data.currentDirective ?? "No public directive."} /><KV k="source" v={data.source ?? "PUBLIC FALLBACK"} /><KV k="last seen" v={data.lastSeen ?? "UNKNOWN"} /></dl>{dossier && <Link className="text-accent underline" href={`/database/${dossier.slug}`}>Open BNL dossier →</Link>}</Block>; }
