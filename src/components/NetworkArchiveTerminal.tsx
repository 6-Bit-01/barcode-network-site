...
const bnlButtons = [{ label: "STATUS", command: "STATUS" }, { label: "TRACE", command: "BNL-01" }, { label: "RELAYS", command: "RELAYS" }, { label: "JOURNAL", command: "BNL LOG" }, { label: "HUB", command: "BNL HUB" }, { label: "BALLADS", command: "BNL BALLADS" }];
...
export function NetworkArchiveTerminal({ archive, restored, onLock }: { archive: ArchivePayload; restored: boolean; onLock: () => void }) {
  ...
  function push(command: string | undefined, node: ReactNode, variant: Entry["variant"] = "normal") {
    ...
    setEntries((prev) => [...prev, { id, command, node, variant }]);
    setScrollVersion((version) => version + 1);
  }

  function pushLive(command: string, liveView: Entry["liveView"]) {
    const id = idRef.current++;
    setEntries((prev) => [...prev, { id, command, liveView }]);
    setScrollVersion((version) => version + 1);
  }
  ...
  function execute(raw: string) {
    ...
    if (command === "TRACE BNL-01" || command === "BNL-01" || command === "BNL") return pushLive(raw, "trace");
    ...
    if (command === "BNL BALLADS" || command === "BALLADS") return push(raw, <Block title="BROADCAST BALLADS"><p>BNL turns BARCODE Radio moments, music and community stories into original songs. Each released Ballad lives with its show, alongside the lyrics, story and linked artists.</p><Link href="/radio/archive" className="text-accent underline">Choose a show in the Broadcast Archive →</Link></Block>);
    if (command === "BNL HUB") return push(raw, <BNLHub />);
    if (command === "STATUS") return pushLive(raw, "status");
...
function TraceBNL({ data, loading, dossier, discordHref }: { data: import("@/components/bnl-status").BNLStatus; loading: boolean; dossier?: ArchiveDossier; discordHref: string }) { return <Block title="TRACE BNL-01"><p>BNL-01 watches the public movement around BARCODE—what the community is discussing, what keeps returning, and what changes around the show—then turns what matters into relays and Journal entries.</p><dl className="grid gap-2 sm:grid-cols-2"><KV k="link state" v={loading ? "Syncing public relay" : data.status} /><KV k="surface reading" v={data.message ?? "No public surface reading."} /><KV k="network posture" v={data.currentDirective ?? "No public network posture."} /><KV k="last transmission" v={lastTransmission(data.lastSeen)} /></dl><p className="flex flex-wrap gap-3"><Link className="text-accent underline" href="/bnl">BNL-01 Hub</Link><a className="text-accent underline" href={discordHref} target="_blank" rel="noreferrer">Talk with BNL in Discord</a><Link className="text-accent underline" href="/radio">BARCODE Radio</Link>{dossier && <Link className="text-accent underline" href={`/database/${dossier.slug}`}>BNL dossier</Link>}</p><p>Terminal reads approved public records. Speaking with BNL happens in Discord.</p></Block>; }
function RelayList({ relays, unavailable }: { relays: ArchiveBNLRelay[]; unavailable: boolean }) { if (unavailable) return <Block title="LIST RELAYS"><p>Public relay history is unavailable right now. Reload the page or return later.</p></Block>; if (!relays.length) return <Block title="LIST RELAYS"><p>No public BNL-01 relays have been published yet.</p></Block>; return <Block title="LIST RELAYS"><div className="space-y-4">{relays.slice(0, 20).map((entry, index) => <div key={`${entry.publishedAt}-${index}`}><p className="text-accent">Relay {index + 1} // <BNLRelayTimestamp value={entry.publishedAt} /></p><p>Surface reading: {entry.message}</p><p>Network posture: {entry.currentDirective}</p></div>)}</div></Block>; }
function BNLLog({ entries, unavailable }: { entries: ArchiveBNLJournalEntry[]; unavailable: boolean }) { if (unavailable) return <Block title="BNL LOG"><p>Public Journal entries are unavailable right now. Reload the page or return later.</p><Link className="text-accent underline" href="/journal">Open full Journal →</Link></Block>; if (!entries.length) return <Block title="BNL LOG"><p>No public Journal entries have been published yet.</p><Link className="text-accent underline" href="/journal">Open full Journal →</Link></Block>; return <Block title="BNL LOG"><div className="space-y-4">{entries.map((entry) => <div key={entry.entryId}><p className="text-accent">{entry.entryKind} // <BNLRelayTimestamp value={entry.publishedAt} /></p><p>{entry.title}</p><p>{entry.excerpt}</p><Link className="text-accent underline" href={entry.href}>Open entry →</Link></div>)}</div><Link className="text-accent underline" href="/journal">Open full Journal →</Link></Block>; }
