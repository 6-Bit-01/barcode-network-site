  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr]">
      ...
      <section className="border border-border bg-surface p-5 space-y-4">
        ...
        {session && session.status !== "archived" && <a href={`/queue/${session.sessionId}`} onClick={(event) => beginNavigation(event, `/queue/${session.sessionId}`)} aria-busy={Boolean(pendingNavigation)} className={`nav-corridor-link inline-flex border px-4 py-3 text-xs uppercase tracking-widest transition-all ${pendingNavigation ? "border-accent bg-accent/10 text-accent" : snapshot?.status.isOpen ? "border-accent text-accent shadow-[0_0_22px_rgba(255,0,0,0.18)] hover:bg-accent hover:text-background" : "border-border text-muted hover:border-accent hover:text-accent"}`}>{pendingNavigation ? pendingNavigation.label : "Enter Session Monitor"}</a>}
      ...
      {mounted && pendingNavigation && createPortal(<div data-kind={pendingNavigation.kind} className="nav-corridor-overlay fixed inset-0 z-[9900] grid place-items-center bg-black/86 p-4 text-center backdrop-blur-sm" role="status" aria-live="polite"><div className="nav-route-lines pointer-events-none absolute inset-0" aria-hidden="true">{Array.from({ length: 10 }).map((_, index) => <span key={index} />)}</div><div className="nav-corridor-panel relative w-full max-w-xl overflow-hidden border border-cyan-200/45 bg-background/92 p-6 shadow-[0_0_90px_rgba(103,232,249,0.18)]"><div className="nav-aperture mx-auto mb-5" aria-hidden="true"><span /><span /><span /></div><p className="text-xs uppercase tracking-[0.4em] text-cyan-200">{pendingNavigation.label}</p><h2 className="mt-3 text-2xl font-bold text-foreground">{pendingNavigation.detail}</h2><p className="mt-2 text-xs uppercase tracking-[0.25em] text-muted">{pendingNavigation.mode} · RECEIVER SCAN LOCK</p><div className="mt-5 grid grid-cols-8 gap-1" aria-hidden="true">{Array.from({ length: 16 }).map((_, index) => <span key={index} className="nav-corridor-bit h-8 border border-cyan-200/30 bg-cyan-200/10" />)}</div></div></div>, document.body)}
