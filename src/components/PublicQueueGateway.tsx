      ...
      <section data-phase={phase} data-pressure={pressure} data-transition={transitionPulse ?? undefined} className={`queue-machine relative overflow-hidden border bg-surface p-5 ${copy.border} ${copy.glow}`}>
        ...
        <div className="relative space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <StatCard label="Remaining" value={counts.active} helper="Still in play until finished or removed." />
...
function AsciiSessionPortalIntro({ label, detail, mode, seed }: { label: string; detail: string; mode: string; seed: string }) {
  ...
  const noise = Array.from({ length: 48 }).map((_, index) => glyphs[stableHash(`${seed}:noise:${index}`) % glyphs.length]);
  ...
  return (
    <div className="ascii-session-overlay fixed inset-0 z-[9900] overflow-hidden bg-black p-4 text-center font-mono text-emerald-200" role="status" aria-live="polite">
      ...
      <div className="ascii-noise pointer-events-none absolute inset-0" aria-hidden="true">{noise.map((char, index) => <span key={index} style={{ left: `${(stableHash(`${seed}:x:${index}`) % 96) + 2}%`, top: `${(stableHash(`${seed}:y:${index}`) % 92) + 4}%`, animationDelay: `${index * 53}ms` }}>{char}</span>)}</div>
      <div className="ascii-calibration pointer-events-none absolute inset-0" aria-hidden="true"><span /><span /><span /><span /></div>
      <div className="ascii-portal absolute left-1/2 top-1/2 h-[min(62vw,62vh)] w-[min(62vw,62vh)] -translate-x-1/2 -translate-y-1/2" aria-hidden="true">
      ...
      <div className="ascii-phase-stack absolute inset-x-4 bottom-[12vh] mx-auto max-w-3xl">
        {phases.map((phase, index) => <p key={phase} className="ascii-phase text-xs uppercase tracking-[0.36em] sm:text-sm" style={{ animationDelay: `${index === 0 ? 0 : [1, 2.2, 3.5, 4.7][index - 1]}s` }}>{phase}</p>)}
        <h2 className="ascii-final mt-4 text-2xl font-bold uppercase tracking-[0.24em] text-foreground sm:text-4xl">{label}</h2>
        <p className="ascii-final mt-3 text-sm uppercase tracking-[0.3em] text-emerald-200">{detail} · {mode}</p>
      </div>
      <style jsx>{`.ascii-session-overlay{animation:ascii-overlay-exit 6s linear forwards}.ascii-crt{background:linear-gradient(transparent 50%,rgba(255,255,255,.055) 50%),radial-gradient(circle at center,rgba(16,185,129,.16),transparent 48%);background-size:100% 6px,100% 100%;animation:ascii-crt-sweep 1.1s linear infinite}.ascii-noise span{position:absolute;opacity:0;color:rgba(110,231,183,.74);animation:ascii-noise 1.2s steps(2,end) infinite}.ascii-calibration span{position:absolute;background:rgba(110,231,183,.26);box-shadow:0 0 18px rgba(110,231,183,.26);animation:ascii-calibrate 6s ease-in-out forwards}.ascii-calibration span:nth-child(1),.ascii-calibration span:nth-child(2){left:50%;top:8%;bottom:8%;width:1px}.ascii-calibration span:nth-child(2){transform:translateX(-50%) rotate(90deg)}.ascii-calibration span:nth-child(3){left:12%;right:12%;top:50%;height:1px}.ascii-calibration span:nth-child(4){top:12%;bottom:12%;left:50%;width:1px}.ascii-portal{perspective:900px;filter:drop-shadow(0 0 34px rgba(16,185,129,.34))}.ascii-ring{position:absolute;inset:0;display:grid;place-items:center;border:1px solid rgba(110,231,183,.28);border-radius:999px;white-space:pre-wrap;line-height:1.8;color:rgba(110,231,183,.86);text-shadow:0 0 12px rgba(110,231,183,.52);animation:ascii-ring-spin 6s cubic-bezier(.18,.82,.22,1) forwards}.ascii-ring-middle{inset:16%;animation-name:ascii-ring-spin-reverse;color:rgba(255,170,0,.72);border-color:rgba(255,170,0,.32)}.ascii-ring-inner{inset:32%;animation-duration:5.2s}.ascii-core{position:absolute;inset:42%;display:grid;place-items:center;border:1px solid rgba(255,255,255,.3);background:rgba(0,0,0,.72);color:#fff;font-size:.65rem;letter-spacing:.3em;animation:ascii-core-open 6s ease-in-out forwards}.ascii-phase{opacity:0;position:absolute;inset-inline:0;bottom:4rem;animation:ascii-phase 1.15s ease-out forwards}.ascii-final{opacity:0;animation:ascii-final 6s ease-out forwards}@keyframes ascii-overlay-exit{0%,94%{opacity:1}100%{opacity:0}}@keyframes ascii-crt-sweep{from{background-position:0 0,0 0}to{background-position:0 48px,0 0}}@keyframes ascii-noise{0%,100%{opacity:.08;transform:scale(.8)}50%{opacity:.65;transform:scale(1.12)}}@keyframes ascii-calibrate{0%{opacity:0;transform:scale(.7)}25%{opacity:.9;transform:scale(1)}78%{opacity:.45}100%{opacity:.12;transform:scale(1.08)}}@keyframes ascii-ring-spin{0%{transform:rotate(0deg) scale(.42);opacity:.18;filter:blur(2px)}18%{opacity:.8}58%{transform:rotate(380deg) scale(1.1);filter:blur(0)}82%{transform:rotate(620deg) scale(1.28);opacity:1}100%{transform:rotate(720deg) scale(1.45);opacity:.15}}@keyframes ascii-ring-spin-reverse{0%{transform:rotate(0deg) scale(.32);opacity:.16}58%{transform:rotate(-420deg) scale(1.04);opacity:.95}100%{transform:rotate(-760deg) scale(1.34);opacity:.12}}@keyframes ascii-core-open{0%,55%{transform:scale(.2);opacity:.12}78%{transform:scale(1.35);opacity:1;box-shadow:0 0 60px rgba(110,231,183,.45)}100%{transform:scale(2.1);opacity:.1}}@keyframes ascii-phase{0%{opacity:0;transform:translateY(8px);filter:blur(3px)}18%,72%{opacity:1;transform:translateY(0);filter:blur(0)}100%{opacity:0;transform:translateY(-8px);filter:blur(2px)}}@keyframes ascii-final{0%,74%{opacity:0;transform:scale(.96);filter:blur(3px)}82%,94%{opacity:1;transform:scale(1);filter:blur(0)}100%{opacity:0;transform:scale(1.02)}}@media (prefers-reduced-motion: reduce){.ascii-session-overlay{animation:ascii-overlay-exit 2s linear forwards}.ascii-crt,.ascii-noise span,.ascii-calibration span,.ascii-ring,.ascii-core,.ascii-phase,.ascii-final{animation:none}.ascii-final{opacity:1}.ascii-phase-stack .ascii-phase:not(:last-of-type){display:none}}`}</style>
    </div>
  );
}
