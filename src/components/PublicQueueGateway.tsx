...
export function PublicQueueGateway() {
  ...
  const [pendingNavigation, setPendingNavigation] = useState<{ href: string; label: string; detail: string } | null>(null);
  ...
  function beginNavigation(event: React.MouseEvent<HTMLAnchorElement>, href: string, isIntake: boolean) {
    ...
    setPendingNavigation({ href, label: isIntake ? "ENTERING INTAKE CORRIDOR" : "ENTERING SESSION MONITOR", detail: isIntake ? "SESSION HANDOFF IN PROGRESS" : "RECEIVER HANDOFF IN PROGRESS" });
