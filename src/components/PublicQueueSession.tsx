...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  const priorityUpgradeInstructions = snapshot?.session.priorityUpgradeInstructions || "Priority Signal Upgrade is being prepared for this session. This placeholder does not charge money or move your track automatically.";
  ...
  async function requestPriorityUpgrade(track: QueuePublicTrack) {
    ...
    setPriorityRequestMessage(res.ok ? payload.message ?? "Placeholder noted. No charge was made." : payload.error ?? "Priority Signal Upgrade is not available right now.");
