...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <ReceiverStatusStrip snapshot={snapshot} canSubmit={canSubmit} isBroadcastActive={isBroadcastActive} pulse={broadcastStartPulse} />
...
function ReceiverStatusStrip({ snapshot, canSubmit, isBroadcastActive, pulse }: { snapshot: QueuePublicSnapshot | null; canSubmit: boolean; isBroadcastActive: boolean; pulse: boolean }) {
  const counts = publicQueueCounts(snapshot);
  const nowPlaying = snapshot?.nowPlaying ?? null;
  const upNext = snapshot?.upNext ?? null;
  return (
    <section data-live={isBroadcastActive ? "true" : "false"} data-pulse={pulse ? "true" : undefined} className="receiver-status-strip sticky z-40 border border-border bg-background/95 p-2 shadow-[0_0_28px_rgba(255,0,0,0.10)] backdrop-blur-md" aria-label="BARCODE receiver status">
      <div className="receiver-strip-scan pointer-events-none absolute inset-0" aria-hidden="true" />
      <div className="relative flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[0.18em] text-muted sm:gap-3">
        <span className="font-bold text-foreground">BARCODE RECEIVER STATUS</span>
        <span className={isBroadcastActive ? "text-[#ffaa00]" : "text-muted"}>Broadcast: {isBroadcastActive ? "Active" : "Standby"}</span>
        <span className={canSubmit ? "text-accent" : "text-muted"}>Submissions: {canSubmit ? "Open" : "Closed"}</span>
        <span>Active: {counts.active}</span>
        {counts.pending > 0 && <span className="text-[#ffaa00]">Payment Processing: {counts.pending}</span>}
        {nowPlaying && <span className="min-w-0 max-w-full truncate text-[#ffaa00] sm:max-w-[22rem]">Now: {nowPlaying.submittedArtistName} — {nowPlaying.submittedSongTitle}</span>}
        {upNext && <span className="min-w-0 max-w-full truncate text-accent sm:max-w-[22rem]">Next: {upNext.submittedArtistName} — {upNext.submittedSongTitle}</span>}
      </div>
      {pulse && <div className="receiver-strip-banner mt-2 border border-[#ffaa00]/40 bg-[#ffaa00]/10 px-2 py-1 text-center text-[10px] font-bold uppercase tracking-[0.28em] text-[#ffaa00]">BROADCAST ACTIVE · HOST PROTOCOL INITIALIZED</div>}
      <style jsx>{`.receiver-status-strip{top:calc(3.75rem + env(safe-area-inset-top));overflow:hidden}.receiver-strip-scan{background:linear-gradient(transparent 50%,rgba(255,255,255,.055) 50%);background-size:100% 6px;opacity:.12}.receiver-status-strip[data-live="true"]{border-color:rgba(255,170,0,.42);box-shadow:0 0 34px rgba(255,170,0,.14)}.receiver-status-strip[data-pulse="true"]{animation:receiver-strip-lock 1.2s ease-out}.receiver-strip-banner{animation:receiver-strip-banner .9s ease-out}@keyframes receiver-strip-lock{0%{filter:brightness(1)}28%{filter:brightness(1.6)}100%{filter:brightness(1)}}@keyframes receiver-strip-banner{0%{opacity:0;transform:translateY(-4px)}100%{opacity:1;transform:translateY(0)}}@media (min-width: 900px){.receiver-status-strip{top:calc(4.5rem + env(safe-area-inset-top))}}@media (prefers-reduced-motion: reduce){.receiver-status-strip[data-pulse="true"],.receiver-strip-banner{animation:none}}`}</style>
    </section>
  );
}
