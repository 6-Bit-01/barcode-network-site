...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <PersonalSignalStatusBar snapshot={snapshot} mounted={mounted} timingSummary={timingSummary} />
      ...
      <section className="space-y-4">
        ...
        <SubmitterOutlookPanel snapshot={snapshot} canSubmit={canSubmit} isFull={isFull} timingSummary={timingSummary} onSubmit={openIntakeCorridor} />
      </section>
...
function SubmitterOutlookPanel({ snapshot, canSubmit, isFull, timingSummary, onSubmit }: { snapshot: QueuePublicSnapshot | null; canSubmit: boolean; isFull: boolean; timingSummary: QueueTimingDisplaySummary | null; onSubmit: () => void }) {
  ...
  return <section className={`submitter-outlook border bg-surface p-5 ${canSubmit ? "border-accent/55 shadow-[0_0_34px_rgba(255,0,0,0.18)]" : "border-border"}`} aria-label="Submitter Outlook"><div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_auto]"><div><p className={`text-xs uppercase tracking-[0.34em] ${statusTone}`}>{statusLabel}</p><h2 className="mt-2 text-2xl font-black uppercase tracking-[0.12em] text-foreground">{canSubmit ? "Submit Your Track Now" : isFull ? "This broadcast queue is full for new songs." : "Public intake is sealed."}</h2>{canSubmit ? <p className="mt-2 text-sm text-muted">Join the free queue. The line updates live as Priority, wheel pulls, removals, and host decisions happen.</p> : <p className="mt-2 text-sm text-muted">Current line: {songsAhead} {songsAhead === 1 ? "song" : "songs"} still waiting or coming up.</p>}<p className="mt-3 border border-border/70 bg-background/45 p-3 text-sm font-bold text-foreground">{canSubmit && estimate ? `If you submit now: ${songsAhead} ${songsAhead === 1 ? "song" : "songs"} ahead · estimated wait ${estimate.label.toLowerCase()}.` : canSubmit ? `If you submit now: ${songsAhead} ${songsAhead === 1 ? "song" : "songs"} ahead.` : `Current line: ${songsAhead} ${songsAhead === 1 ? "song" : "songs"} still waiting or coming up.`}</p>{canSubmit && timingSummary?.publicNotes.length ? <div className="mt-2 space-y-1 text-xs text-muted">{timingSummary.publicNotes.map((note) => <p key={note}>{note}</p>)}</div> : null}</div>{canSubmit && <div className="flex items-end lg:min-w-[16rem]"><button type="button" onClick={onSubmit} className="intake-cta w-full border border-accent bg-accent/10 px-5 py-4 text-sm font-black uppercase tracking-[0.22em] text-accent shadow-[0_0_28px_rgba(255,0,0,0.22)] hover:bg-accent hover:text-background">Submit Track</button></div>}</div><div className={`mt-5 grid gap-3 ${queueSpace === null ? "sm:grid-cols-3" : "sm:grid-cols-4"}`}><QueueStat label="Songs Ahead" value={songsAhead} helper="Songs currently ahead of a new submission." />{estimate && <QueueStat label="Estimated Wait" value={estimate.label} helper="A rough range, not an exact play time." accent="text-accent" />}{queueSpace !== null && <QueueStat label="Queue Space" value={`${queueSpace} ${queueSpace === 1 ? "spot" : "spots"}`} helper="Room left in tonight’s submission line." accent={queueSpace > 0 ? "text-foreground" : "text-danger"} />}<QueueStat label={timingSummary?.lineFitStatus && timingSummary.lineFitStatus !== "unknown" ? "Line Fit" : "Live Line"} value={lineFit} helper={liveCopy} accent="text-accent" /></div><style jsx>{`.intake-cta{animation:intake-cta-pulse 2.1s ease-in-out infinite}@keyframes intake-cta-pulse{0%,100%{box-shadow:0 0 16px rgba(255,0,0,.12)}50%{box-shadow:0 0 38px rgba(255,0,0,.34)}}@media (prefers-reduced-motion: reduce){.intake-cta{animation:none}}`}</style></section>;
}
...
function PersonalSignalStatusBar({ snapshot, mounted, timingSummary }: { snapshot: QueuePublicSnapshot | null; mounted: boolean; timingSummary: QueueTimingDisplaySummary | null }) {
  ...
  if (status && status.used > 0) {
    ...
    const priorityActiveTrack = allSubmitted.find((track) => "priorityUpgradeStatus" in track && (track.priorityUpgradeStatus === "paid" || track.priorityUpgradeStatus === "manual"));
