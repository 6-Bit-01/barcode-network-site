  ...
  return (
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        ...
        <SubmitterOutlookPanel snapshot={snapshot} canSubmit={canSubmit} isFull={isFull} onSubmit={openIntakeCorridor} />
      </section>
...
function SubmitterOutlookPanel({ snapshot, canSubmit, isFull, onSubmit }: { snapshot: QueuePublicSnapshot | null; canSubmit: boolean; isFull: boolean; onSubmit: () => void }) {
  const counts = publicQueueCounts(snapshot);
  const songsAhead = counts.remaining;
  const roughWaitMinutes = songsAhead * 5;
  const roughWait = songsAhead > 0 ? `About ${roughWaitMinutes} min` : "Near the front";
  const lineSummary = songsAhead > 0 ? `${songsAhead} ${songsAhead === 1 ? "song" : "songs"} ahead · estimated wait about ${roughWaitMinutes} min.` : "No songs currently ahead · near the front.";
