  ...
  const hud = (
    <section data-live={isBroadcastActive ? "true" : "false"} data-pulse={pulse ? "true" : undefined} className="receiver-hud fixed inset-x-3 z-[9000] border border-border bg-background/95 p-2 shadow-[0_0_28px_rgba(255,0,0,0.10)] backdrop-blur-md sm:inset-x-4" aria-label="BARCODE receiver status">
      ...
      <div className="relative flex items-center gap-2 overflow-hidden text-[10px] uppercase tracking-[0.18em] text-muted sm:gap-3">
        ...
        {minimized ? null : <span className="shrink-0">Remaining: {counts.remaining}</span>}
        {!minimized && canSubmit && <button type="button" onClick={onSubmit} className="border border-accent/60 bg-accent/10 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.2em] text-accent hover:bg-accent hover:text-background">Submit Track</button>}
        {!minimized && !canSubmit && <span className="border border-border px-2 py-1 text-[10px]">{submitLabel}</span>}
        <button type="button" onClick={onToggleMinimized} className="ml-auto border border-border px-2 py-1 text-[10px] uppercase tracking-[0.2em] text-muted hover:text-foreground">{minimized ? "Expand" : "Minimize"}</button>
        {!minimized && counts.pending > 0 && <span className="shrink-0 text-[#ffaa00]">Payment Processing: {counts.pending}</span>}
        {!minimized && nowPlaying && <span className="min-w-[8rem] truncate text-[#ffaa00]">Now: {nowPlaying.submittedArtistName} — {nowPlaying.submittedSongTitle}</span>}
        {!minimized && upNext && <span className="hidden min-w-[8rem] truncate text-accent sm:inline">Next: {upNext.submittedArtistName} — {upNext.submittedSongTitle}</span>}
      </div>
