...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  const [actionTransition, setActionTransition] = useState<{ label: string; detail: string } | null>(null);
  ...
  function runPublicActionTransition(label: string, detail: string, action: () => void, delay = 620) {
    setActionTransition({ label, detail });
    window.setTimeout(() => {
  ...
  function openIntakeCorridor() {
    runPublicActionTransition("ENTERING INTAKE CORRIDOR", "SESSION HANDOFF IN PROGRESS", () => { setIntakeScrollLocked(true); setSubmitOpen(true); });
  }

  function requestPriorityUpgrade(track: QueuePublicTrack) {
    runPublicActionTransition("PRIORITY RELAY REQUESTED", "OPENING SECURE CHECKOUT", () => {
  ...
  async function resumePriorityPayment(track: QueuePublicTrack) {
    ...
    setPriorityRequestMessage(null);
    setActionTransition({ label: "RESUMING SECURE CHECKOUT", detail: "PRIORITY RELAY STILL PENDING" });
  ...
  return (
    <div className="space-y-6">
      ...
      {mounted && actionTransition && createPortal(<NavigationTransition label={actionTransition.label} detail={actionTransition.detail} />, document.body)}
