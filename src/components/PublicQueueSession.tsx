...
function PersonalSignalStatusBar({ snapshot, mounted, timingSummary, minimized, onToggleMinimized, canSubmit, submitLabel, onSubmit }: { snapshot: QueuePublicSnapshot | null; mounted: boolean; timingSummary: QueueTimingDisplaySummary | null; minimized: boolean; onToggleMinimized: () => void; canSubmit: boolean; submitLabel: string; onSubmit: () => void }) {
  ...
  if (status && status.used > 0) {
    ...
    const counts = status.lifecycleCounts;
    const playedCount = counts?.finishedTrackCount ?? played.length;
    const waitingCount = counts?.waitingTrackCount ?? waiting.length;
    const countDetail = counts ? `${waitingCount} waiting · ${playedCount} finished · ${counts.skippedTrackCount} skipped · ${counts.removedTrackCount} removed.` : `${waitingCount} waiting · ${playedCount} recently played.`;
    ...
    const activeIds = new Set([snapshot?.nowPlaying?.id, snapshot?.upNext?.id, ...waiting.map((track) => track.id)].filter(Boolean));
    const priorityActiveTrack = allSubmitted.find((track) => activeIds.has(track.id) && isActivePublicPriority(track));
    ...
    main = `${status.used} of ${status.limit} submission slots used.`;
    detail = "Removed tracks still count toward the show submission limit.";
    ...
    else if (waitingCount === 0 && playedCount > 0) { main = `${pluralizeSongs(playedCount)} finished tonight.`; detail = "Recently Played shows the latest tracks; your counts cover the full show."; }
