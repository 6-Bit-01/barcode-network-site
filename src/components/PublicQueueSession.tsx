...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  const priorityUpgradeLabel = snapshot?.session.priorityUpgradeLabel || "Upgrade to Priority Signal";
  const priorityUpgradeInstructions = snapshot?.session.priorityUpgradeInstructions || "Move this transmission into the Priority Signal lane after payment confirmation.";
...
function PriorityUpgradeModal({ track, label, instructions, pending, message, onClose }: { track: QueuePublicTrack; label: string; instructions: string; pending: boolean; message: string | null; onClose: () => void }) {
  return <div className="fixed inset-0 z-[10050] grid place-items-center bg-black/80 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-md border border-[#ffaa00]/50 bg-background p-5 shadow-[0_0_70px_rgba(255,170,0,0.22)]"><p className="text-xs uppercase tracking-[0.35em] text-[#ffaa00]">{label}</p><h2 className="mt-3 text-2xl font-bold text-foreground">Upgrade to Priority Signal</h2><p className="mt-2 text-sm text-muted">{track.submittedArtistName} — {track.submittedSongTitle}</p><p className="mt-3 text-sm text-muted">{instructions}</p><div className="mt-4 border border-border bg-surface p-3 text-xs text-muted"><p>Move this transmission into the Priority Signal lane after payment confirmation.</p><p>Payment confirmation may take a moment.</p><p>The checkout success page cannot move a track; only the verified payment webhook can.</p></div>{message && <p className="mt-3 border border-accent/30 bg-accent/5 p-2 text-xs text-accent">{message}</p>}<div className="mt-5 flex justify-end"><button type="button" onClick={onClose} disabled={pending} className="border border-[#ffaa00]/60 px-4 py-2 text-xs uppercase tracking-widest text-[#ffaa00] disabled:opacity-50">{pending ? "Opening checkout…" : "Close"}</button></div></div></div>;
}
