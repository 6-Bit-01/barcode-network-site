...
function ReceiverStatusStrip({ snapshot, canSubmit, isBroadcastActive, pulse }: { snapshot: QueuePublicSnapshot | null; canSubmit: boolean; isBroadcastActive: boolean; pulse: boolean }) {
  ...
  return (
    <>
      <section data-live={isBroadcastActive ? "true" : "false"} data-pulse={pulse ? "true" : undefined} className="receiver-status-strip fixed inset-x-3 z-40 border border-border bg-background/95 p-2 shadow-[0_0_28px_rgba(255,0,0,0.10)] backdrop-blur-md sm:inset-x-4" aria-label="BARCODE receiver status">
        ...
        <div className="relative flex items-center gap-2 overflow-hidden text-[10px] uppercase tracking-[0.18em] text-muted sm:gap-3">
          ...
          <span className={`shrink-0 ${canSubmit ? "text-accent" : "text-muted"}`}>Submissions: {canSubmit ? "Open" : "Closed"}</span>
          <span className="shrink-0">Active: {counts.active}</span>
      ...
      <div className="receiver-status-spacer" data-pulse={pulse ? "true" : undefined} aria-hidden="true" />
      <style jsx>{`.receiver-status-strip{top:calc(3.75rem + env(safe-area-inset-top));max-height:5.5rem;overflow:hidden}.receiver-status-spacer{height:3.75rem}.receiver-status-spacer[data-pulse="true"]{height:5.75rem}.receiver-strip-scan{background:linear-gradient(transparent 50%,rgba(255,255,255,.055) 50%);background-size:100% 6px;opacity:.12}.receiver-status-strip[data-live="true"]{border-color:rgba(255,170,0,.42);box-shadow:0 0 34px rgba(255,170,0,.14)}.receiver-status-strip[data-pulse="true"]{animation:receiver-strip-lock 1.2s ease-out}.receiver-strip-banner{animation:receiver-strip-banner .9s ease-out}@keyframes receiver-strip-lock{0%{filter:brightness(1)}28%{filter:brightness(1.6)}100%{filter:brightness(1)}}@keyframes receiver-strip-banner{0%{opacity:0;transform:translateY(-4px)}100%{opacity:1;transform:translateY(0)}}@media (min-width: 900px){.receiver-status-strip{top:calc(4.5rem + env(safe-area-inset-top))}.receiver-status-spacer{height:4rem}.receiver-status-spacer[data-pulse="true"]{height:6rem}}@media (prefers-reduced-motion: reduce){.receiver-status-strip[data-pulse="true"],.receiver-strip-banner{animation:none}}`}</style>
    </>
  );
}
