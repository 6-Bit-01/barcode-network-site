...
function ReceiverHudPortal({ snapshot, submissionsOpen, isBroadcastActive, pulse, mounted, minimized, onToggleMinimized, canSubmit, submitLabel, onSubmit }: { snapshot: QueuePublicSnapshot | null; submissionsOpen: boolean; isBroadcastActive: boolean; pulse: boolean; mounted: boolean; minimized: boolean; onToggleMinimized: () => void; canSubmit: boolean; submitLabel: string; onSubmit: () => void }) {
  ...
  return <><div className="receiver-hud-spacer" data-pulse={pulse ? "true" : undefined} data-minimized={minimized ? "true" : undefined} aria-hidden="true" />{mounted && createPortal(hud, document.body)}<style jsx>{`.receiver-hud{top:calc(10.75rem + env(safe-area-inset-top));overflow:visible}.receiver-hud[data-minimized="true"]{padding:.5rem .6rem}.receiver-hud-spacer{height:6.3rem}.receiver-hud-spacer[data-minimized="true"]{height:4.2rem}.receiver-hud-spacer[data-pulse="true"]{height:7.4rem}.receiver-hud-scan{background:linear-gradient(transparent 50%,rgba(255,255,255,.055) 50%);background-size:100% 6px;opacity:.12}.receiver-hud[data-live="true"]{border-color:rgba(255,170,0,.42);box-shadow:0 0 34px rgba(255,170,0,.14)}.receiver-hud[data-pulse="true"]{animation:receiver-hud-lock 1.2s ease-out}.receiver-hud-packet{animation:receiver-hud-packet 1.4s ease-in-out infinite}.receiver-hud-banner{animation:receiver-hud-banner .9s ease-out}@keyframes receiver-hud-lock{0%{filter:brightness(1)}28%{filter:brightness(1.6)}100%{filter:brightness(1)}}@keyframes receiver-hud-banner{0%{opacity:0;transform:translateY(-4px)}100%{opacity:1;transform:translateY(0)}}@keyframes receiver-hud-packet{0%,100%{opacity:.45;transform:scale(.85)}50%{opacity:1;transform:scale(1.18)}}@media (min-width: 900px){.receiver-hud{top:calc(11.5rem + env(safe-area-inset-top))}.receiver-hud-spacer{height:6.8rem}.receiver-hud-spacer[data-minimized="true"]{height:4.3rem}.receiver-hud-spacer[data-pulse="true"]{height:8rem}}@media (prefers-reduced-motion: reduce){.receiver-hud[data-pulse="true"],.receiver-hud-banner,.receiver-hud-packet{animation:none}}`}</style></>;
}
...
function PersonalSignalStatusBar({ snapshot, mounted, timingSummary, minimized, onToggleMinimized, canSubmit, submitLabel, onSubmit }: { snapshot: QueuePublicSnapshot | null; mounted: boolean; timingSummary: QueueTimingDisplaySummary | null; minimized: boolean; onToggleMinimized: () => void; canSubmit: boolean; submitLabel: string; onSubmit: () => void }) {
  ...
  const band = <section className="personal-signal-bar fixed inset-x-0 z-[9050] w-screen overflow-hidden border-b border-accent/35 border-t border-border/70 bg-black/95 font-mono text-white backdrop-blur-md" aria-label="Your Signal Status" role="status" aria-live="polite"><div className="personal-signal-scan pointer-events-none absolute inset-0" aria-hidden="true" /><div className="personal-signal-line pointer-events-none absolute inset-x-0 bottom-0 h-px bg-accent/55" aria-hidden="true" /><div className="relative mx-auto flex min-h-[5rem] max-w-7xl flex-col justify-center gap-2 px-3 py-3 sm:min-h-[6rem] sm:px-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="text-[10px] font-bold uppercase tracking-[0.34em] text-accent sm:text-xs">Your Signal Status</p>{!minimized && <p className="mt-1 truncate text-sm font-bold text-foreground sm:text-lg">{main}</p>}</div><div className="flex flex-wrap items-center gap-2">{!minimized && canSubmit && <button type="button" onClick={onSubmit} className="border border-accent/60 bg-accent/10 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-[0.2em] text-accent hover:bg-accent hover:text-background">Submit Track</button>}{!minimized && !canSubmit && <span className="border border-border px-2.5 py-1.5 text-[10px] uppercase tracking-[0.2em] text-muted">{submitLabel}</span>}<button type="button" onClick={onToggleMinimized} className="border border-border px-2.5 py-1.5 text-[10px] uppercase tracking-[0.2em] text-muted hover:text-foreground">{minimized ? "Expand" : "Minimize"}</button></div></div>{!minimized && <p className="line-clamp-2 text-[11px] leading-snug text-muted sm:text-sm">{detail}</p>}</div><style jsx>{`.personal-signal-bar{top:calc(5.375rem + env(safe-area-inset-top))}.personal-signal-spacer{height:5.75rem}.personal-signal-scan{background:linear-gradient(transparent 50%,rgba(255,255,255,.06) 50%);background-size:100% 6px;opacity:.14}@media (min-width:900px){.personal-signal-spacer{height:6.4rem}}@media (max-width:640px){.personal-signal-bar p{max-width:100%}.personal-signal-spacer{height:5.4rem}}`}</style></section>;
