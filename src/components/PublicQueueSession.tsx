    ...
    for (const track of publicTrackStateMap(next).values()) {
      ...
      if (before.priorityUpgradeStatus !== track.priorityUpgradeStatus) {
        ...
        if (track.priorityUpgradeStatus === "paid" || track.priorityUpgradeStatus === "paid_needs_attention" || track.priorityUpgradeStatus === "manual") {
          const gift = track.priorityGiftAttribution;
          changes.push({ text: gift ? "Gifted Priority Signal confirmed" : "Priority Signal confirmed", detail: gift ? `From ${gift.supporterName} for ${gift.recipientName}. Thank you for the skip.` : "Payment cleared. Priority Signal active.", tone: "gold" });
...
function PriorityGiftTag({ track, className = "" }: { track: QueuePublicTrack | null; className?: string }) {
  const gift = track?.priorityGiftAttribution;
  if (!gift) return null;
  return <p className={`inline-flex max-w-full flex-wrap border border-[#ffaa00]/70 bg-[#ffaa00]/15 px-2 py-1 text-[10px] font-black uppercase tracking-widest text-[#ffaa00] ${className}`}>FROM {gift.supporterName} · FOR {gift.recipientName}</p>;
}
...
function trackStatusStyle(track: QueuePublicTrack, isLanding: boolean, isCompleted: boolean): { card: string; stamp: string | null } {
  const giftTag = track.priorityGiftAttribution ? ` · SKIP FROM ${track.priorityGiftAttribution.supporterName} · FOR ${track.priorityGiftAttribution.recipientName}` : "";
  if (isCompleted) return { card: "completed-stamp border-border opacity-85", stamp: `ALREADY PLAYED${giftTag}` };
  ...
  if (track.lane === "priority" || track.priorityUpgradeStatus === "manual" || track.priorityUpgradeStatus === "paid" || track.priorityUpgradeStatus === "paid_needs_attention") return { card: "priority-lock border-[#ffaa00]/55", stamp: `PAYMENT CLEARED · PRIORITY SIGNAL ACTIVE${giftTag}` };
