...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {view === "active" ? <div id="active-queue-panel" className="space-y-3"><PublicLane title="Priority Signal" tracks={lanes.priority} lastSubmittedTrackId={lastSubmittedTrackId} collapsible domId="priority-lane" priorityUpgradeEnabled={false} priorityUpgradeLabel={priorityUpgradeLabel} priorityUpgradeInstructions={priorityUpgradeInstructions} onPriorityUpgrade={requestPriorityUpgrade} /><PublicLane title="Wheel Chosen" subtitle="Tracks selected by the 10K tap wheel." tracks={lanes.wheel} lastSubmittedTrackId={lastSubmittedTrackId} collapsible domId="wheel-lane" priorityUpgradeEnabled={priorityUpgradeEnabled} priorityUpgradeLabel={priorityUpgradeLabel} priorityUpgradeInstructions={priorityUpgradeInstructions} onPriorityUpgrade={requestPriorityUpgrade} /><PublicLane title="Free Transmissions" tracks={lanes.regular} lastSubmittedTrackId={lastSubmittedTrackId} domId="free-transmissions-lane" priorityUpgradeEnabled={priorityUpgradeEnabled} priorityUpgradeLabel={priorityUpgradeLabel} priorityUpgradeInstructions={priorityUpgradeInstructions} onPriorityUpgrade={requestPriorityUpgrade} /></div> : <PublicLane title="Recently Played" tracks={snapshot?.completed ?? []} lastSubmittedTrackId={null} priorityUpgradeEnabled={false} priorityUpgradeLabel={priorityUpgradeLabel} priorityUpgradeInstructions={priorityUpgradeInstructions} onPriorityUpgrade={requestPriorityUpgrade} />}

      <DiscordQueueCTA />

      {mounted && priorityModalTrack && createPortal(<PriorityUpgradeModal track={priorityModalTrack} label={priorityUpgradeLabel} instructions={priorityUpgradeInstructions} pending={priorityRequestPending} message={priorityRequestMessage} onClose={() => setPriorityModalTrack(null)} />, document.body)}
...
function PublicLane({ title, tracks, subtitle, lastSubmittedTrackId, collapsible = false, domId, priorityUpgradeEnabled, priorityUpgradeLabel, onPriorityUpgrade }: { title: string; tracks: QueuePublicTrack[]; subtitle?: string; lastSubmittedTrackId: string | null; collapsible?: boolean; domId?: string; priorityUpgradeEnabled: boolean; priorityUpgradeLabel: string; priorityUpgradeInstructions: string; onPriorityUpgrade: (track: QueuePublicTrack) => void }) {
  const collapsed = collapsible && tracks.length === 0;
  const id = domId ?? (title === "Free Transmissions" ? "free-transmissions-lane" : undefined);
  return <section id={id} className={`w-full border bg-surface transition-all ${lastSubmittedTrackId && tracks.some((track) => track.id === lastSubmittedTrackId) ? "border-accent shadow-[0_0_34px_rgba(255,0,0,0.22)]" : "border-border"} ${collapsed ? "p-3" : "p-5"}`}><div className="flex items-start justify-between gap-3"><div><h2 className="text-sm uppercase tracking-[0.25em] text-foreground">{title}</h2>{subtitle && !collapsed && <p className="mt-1 text-xs text-muted">{subtitle}</p>}</div><span className="text-xs text-muted">{tracks.length}</span></div>{collapsed ? <p className="mt-1 text-xs text-muted">No active signals in this lane.</p> : <div className="mt-4 space-y-3">{tracks.length === 0 ? <p className="border border-border/60 p-4 text-sm text-muted">No visible transmissions.</p> : tracks.map((track, index) => <article key={track.id} data-track-id={track.id} id={`track-card-${track.id}`} className={`grid gap-3 border bg-background/40 p-3 sm:grid-cols-[5rem_1fr_auto] sm:items-center ${track.id === lastSubmittedTrackId ? "border-accent animate-pulse" : "border-border"}`}><div className="h-20 overflow-hidden border border-border/70"><SourceArt track={track} /></div><div><p className="text-xs text-muted">#{index + 1} · {track.sourceType.toUpperCase()}</p><p className="font-bold text-foreground">{track.submittedArtistName}</p>{track.publicSourceUrl ? <a href={track.publicSourceUrl} target="_blank" rel="noreferrer" className="text-sm text-foreground/85 underline-offset-2 hover:text-accent hover:underline">{track.submittedSongTitle}</a> : <p className="text-sm text-foreground/85">{track.submittedSongTitle}</p>}<div className="mt-2 text-xs text-muted"><TikTokLink handle={track.tiktokHandle} /></div>{(track.lane === "priority" || track.priorityUpgradeStatus === "manual" || track.priorityUpgradeStatus === "paid") && <p className="mt-2 inline-flex border border-[#ffaa00]/40 px-2 py-1 text-[10px] uppercase tracking-widest text-[#ffaa00]">Priority upgraded</p>}{track.priorityUpgradeStatus === "checkout_pending" && <p className="mt-2 inline-flex border border-[#ffaa00]/40 px-2 py-1 text-[10px] uppercase tracking-widest text-[#ffaa00]">Priority pending</p>}{track.priorityUpgradeStatus === "requested" && <p className="mt-2 inline-flex border border-accent/30 px-2 py-1 text-[10px] uppercase tracking-widest text-accent">Priority requested</p>}{track.id === lastSubmittedTrackId && <p className="mt-2 text-[11px] uppercase tracking-widest text-accent">QUEUE INSERTION CONFIRMED</p>}</div><div className="space-y-2 sm:text-right"><p className="text-xs text-muted">{track.durationLabel}</p>{priorityUpgradeEnabled && track.lane !== "priority" && <button type="button" onClick={() => onPriorityUpgrade(track)} className="border border-[#ffaa00]/50 px-3 py-1.5 text-[10px] uppercase tracking-widest text-[#ffaa00] hover:bg-[#ffaa00] hover:text-background">{priorityUpgradeLabel}</button>}</div></article>)}</div>}</section>;
}

function PriorityUpgradeModal({ track, label, instructions, pending, message, onClose }: { track: QueuePublicTrack; label: string; instructions: string; pending: boolean; message: string | null; onClose: () => void }) {
  return <div className="fixed inset-0 z-[10050] grid place-items-center bg-black/80 p-4 backdrop-blur-sm"><div role="dialog" aria-modal="true" className="w-full max-w-md border border-[#ffaa00]/50 bg-background p-5 shadow-[0_0_70px_rgba(255,170,0,0.22)]"><p className="text-xs uppercase tracking-[0.35em] text-[#ffaa00]">{label}</p><h2 className="mt-3 text-2xl font-bold text-foreground">Priority upgrade is being prepared.</h2><p className="mt-2 text-sm text-muted">{track.submittedArtistName} — {track.submittedSongTitle}</p><p className="mt-3 text-sm text-muted">{instructions}</p><div className="mt-4 border border-border bg-surface p-3 text-xs text-muted"><p>Priority Signal Upgrade is being prepared.</p><p>No payment has been processed.</p><p>Your track will not move lanes from this placeholder alone; admin/manual action is required.</p></div>{message && <p className="mt-3 border border-accent/30 bg-accent/5 p-2 text-xs text-accent">{message}</p>}<div className="mt-5 flex justify-end"><button type="button" onClick={onClose} disabled={pending} className="border border-[#ffaa00]/60 px-4 py-2 text-xs uppercase tracking-widest text-[#ffaa00] disabled:opacity-50">{pending ? "Preparing…" : "Close"}</button></div></div></div>;
}
