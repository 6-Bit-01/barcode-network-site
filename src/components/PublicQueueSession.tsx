...
const PRIORITY_SIGNAL_EXPLANATION = "Paid skip. Moves your song closer to the front after payment clears.";
...
export function PublicQueueSession({ sessionId }: { sessionId: string }) {
  ...
  function processSnapshotChanges(previous: QueuePublicSnapshot | null, next: QueuePublicSnapshot) {
    ...
    if (nextWheelSpinsOwed > previousWheelSpinsOwed) {
      changes.push({ text: "10K Tap Wheel Unlocked", detail: "Host will choose a track.", tone: "cyan" });
    ...
    for (const track of next.queue) {
      ...
      if (before.priorityUpgradeStatus !== track.priorityUpgradeStatus) {
        ...
        if (track.priorityUpgradeStatus === "paid" || track.priorityUpgradeStatus === "manual") {
          changes.push({ text: "Priority Signal confirmed", detail: track.priorityUpgradeStatus === "paid" ? "Payment cleared. Priority Signal active." : "Payment cleared. Priority Signal active.", tone: "gold" });
    ...
    <div className="space-y-6">
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-1">
          ...
          <QueueStatusPanel snapshot={snapshot} canSubmit={canSubmit} isFull={isFull} onSubmit={openIntakeCorridor} wheelPulse={wheelUnlockPulse} />
        </div>
      </section>
...
function SessionPortalAperture({ phase, counts, intakeWindow, isBroadcastActive, canSubmit }: { phase: PublicSessionPhase; counts: ReturnType<typeof publicQueueCounts>; intakeWindow: string | null; isBroadcastActive: boolean; canSubmit: boolean }) {
  ...
  return (
    <div className="session-portal mt-3 overflow-hidden border border-border bg-background/50 p-3" data-phase={phase}>
      <div className="session-aperture relative aspect-[16/9] min-h-44 overflow-hidden border border-border/70 bg-black/35 sm:min-h-52">
...
function NowPlaying({ title, track, compact = false, domId, lastSubmittedTrackId, residue }: { title: string; track: QueuePublicTrack | null; compact?: boolean; domId?: string; lastSubmittedTrackId?: string | null; residue?: { tone: ActivityTone; nonce: number } }) {
  ...
  return <div id={domId} data-track-id={track?.id} data-track-card={track ? "true" : undefined} data-track-zone={isNowPlaying ? "now" : "next"} data-track-tone={track ? trackFlipTone(track, isNowPlaying ? "now" : "next") : undefined} data-track-artist={track?.submittedArtistName} data-track-title={track?.submittedSongTitle} data-track-label={isNowPlaying ? "Now Playing" : "Next In Line"} className={`broadcast-slot relative overflow-hidden border bg-surface p-4 transition-all ${isNowPlaying ? "broadcast-now border-[#ffaa00]/50 shadow-[0_0_34px_rgba(255,170,0,0.14)]" : "broadcast-next border-accent/40"} ${isLanding ? "packet-lock border-accent shadow-[0_0_44px_rgba(255,0,0,0.34)]" : ""} ${compact ? "" : "min-h-[15rem]"}`}><div className="slot-scan pointer-events-none absolute inset-0" />{track && residue && <SignalResidue tone={residue.tone} seed={`${track.id}:${residue.nonce}`} />}<p className={`relative text-xs uppercase tracking-[0.35em] ${tone}`} title={isNowPlaying ? "Playing now." : "Coming up next."}>{title}</p><p className="relative mt-1 text-[11px] text-muted">{isNowPlaying ? "Playing now." : "Coming up next."}</p>{track ? <div className={`relative mt-3 grid gap-4 ${compact ? "grid-cols-[5.5rem_1fr]" : "sm:grid-cols-[12rem_1fr]"}`}><div className={`${compact ? "h-24" : "aspect-square max-h-56"} overflow-hidden border ${isNowPlaying ? "border-[#ffaa00]/45" : "border-accent/40"}`}><SourceArt track={track} /></div><div className="self-center"><h3 className={`${compact ? "text-xl" : "text-3xl"} font-bold text-foreground`}>{track.submittedArtistName}</h3><TrackTitleLink track={track} /><div className="mt-3 grid gap-1 text-xs text-muted"><TikTokLink handle={track.tiktokHandle} />{detected && <p>Detected signal: {detected}</p>}{!track.durationIsEstimate && <p>Runtime locked: {track.durationLabel}</p>}</div>{isNowPlaying && <div className="live-meter mt-4 grid grid-cols-12 items-end gap-1" aria-hidden="true">{[20, 46, 32, 70, 38, 82, 28, 56, 44, 76, 34, 62].map((height, index) => <span key={index} className="bg-[#ffaa00]/70" style={{ height: `${height / 3}px`, animationDelay: `${index * 55}ms` }} />)}</div>}</div></div> : <div className="relative mt-3 grid gap-4 sm:grid-cols-[8rem_1fr]"><div className="h-32 overflow-hidden border border-accent/30"><SourceArt track={null} /></div><p className="self-center text-sm text-muted">No song is in this slot yet.</p></div>}<style jsx>{`.slot-scan{background:linear-gradient(transparent 50%,rgba(255,255,255,.06) 50%);background-size:100% 6px;opacity:.12}.broadcast-now .slot-scan{animation:slot-scan 2.2s linear infinite}.broadcast-next{animation:next-slot-pulse 2.8s ease-in-out infinite}.packet-lock{animation:packet-card-lock 900ms ease-out}.live-meter span{animation:live-meter 900ms ease-in-out infinite}@keyframes slot-scan{from{background-position:0 0}to{background-position:0 42px}}@keyframes next-slot-pulse{0%,100%{box-shadow:0 0 0 rgba(255,0,0,0)}50%{box-shadow:0 0 22px rgba(255,0,0,.12)}}@keyframes packet-card-lock{0%{transform:translateY(4px);filter:brightness(1.5)}100%{transform:translateY(0);filter:brightness(1)}}@keyframes live-meter{0%,100%{transform:scaleY(.45);opacity:.45}50%{transform:scaleY(1);opacity:1}}@media (prefers-reduced-motion: reduce){.broadcast-now .slot-scan,.broadcast-next,.packet-lock,.live-meter span{animation:none}}`}</style></div>;
}

function QueueStatusPanel({ snapshot, canSubmit, isFull, onSubmit, wheelPulse }: { snapshot: QueuePublicSnapshot | null; canSubmit: boolean; isFull: boolean; onSubmit: () => void; wheelPulse: boolean }) {
  const counts = publicQueueCounts(snapshot);
  const wheelSpinsWaiting = snapshot?.session.wheelSpinsOwed ?? 0;
  return <div className={`border bg-surface p-4 space-y-3 ${canSubmit ? "border-accent/45 shadow-[0_0_28px_rgba(255,0,0,0.14)]" : "border-border"}`}><div><p className="text-xs uppercase tracking-[0.3em] text-muted">// Queue Status</p><h2 className="mt-1 text-xl font-bold text-foreground">{snapshot?.session.title ?? "BARCODE Radio"}</h2><p className="text-xs text-muted">{snapshot?.session.showDate ?? "show date syncing"} · {snapshot?.session.status ?? "syncing"}</p></div><div className="grid grid-cols-2 gap-2 text-sm"><QueueStat label="Remaining" value={counts.remaining} helper="Songs still waiting or coming up." /><QueueStat label="Waiting Below" value={counts.waiting} helper="Songs below Now Playing and Next In Line." />{wheelSpinsWaiting > 0 && <QueueStat label="Wheel Spins Waiting" value={wheelSpinsWaiting} helper="Host will choose from the 10K tap wheel." accent="text-cyan-200" pulse={wheelPulse} />}<QueueStat label="Total Received" value={counts.total} helper="Songs submitted this session." /><QueueStat label="Played Tonight" value={counts.completed} helper="Songs already played." />{counts.pending > 0 && <QueueStat label="Payment Processing" value={counts.pending} helper="Checkout started. Skip is not active yet." accent="text-[#ffaa00]" />}</div>{wheelSpinsWaiting > 0 && <div data-pulse={wheelPulse ? "true" : undefined} className="wheel-waiting-callout pointer-events-none overflow-hidden border border-cyan-200/45 bg-cyan-200/5 p-3"><p className="text-[10px] uppercase tracking-[0.3em] text-cyan-200">10K Tap Wheel Unlocked</p><p className="mt-1 text-sm font-bold text-foreground">Wheel spin is waiting.</p><p className="mt-1 text-xs text-muted">Host will choose a track. This does not mean Next In Line changed.</p></div>}{canSubmit ? <><button type="button" onClick={onSubmit} className="intake-cta w-full border border-accent px-4 py-2.5 text-xs uppercase tracking-widest text-accent shadow-[0_0_24px_rgba(255,0,0,0.16)] hover:bg-accent hover:text-background">Submit Track</button><p className="text-[11px] text-muted">Send your song into the free queue.</p></> : <div className="border border-danger/40 bg-danger/5 p-3"><p className="text-xs uppercase tracking-[0.25em] text-danger">{isFull ? "QUEUE FULL" : "SUBMISSIONS CLOSED"}</p><p className="mt-1 text-xs text-muted">{isFull ? "This broadcast queue is full for new songs." : "Visible queue. No new songs accepted right now."}</p></div>}<p className="text-[11px] leading-relaxed text-muted">Remaining means songs still waiting or coming up. Payment Processing means checkout started, but the skip is not active yet.</p><style jsx>{`.intake-cta{animation:intake-cta-pulse 2.1s ease-in-out infinite}.wheel-waiting-callout{position:relative}.wheel-waiting-callout::before{content:"";position:absolute;right:.75rem;top:50%;width:3.6rem;height:3.6rem;border:1px solid rgba(103,232,249,.55);border-radius:999px;transform:translateY(-50%);box-shadow:0 0 28px rgba(103,232,249,.22),inset 0 0 18px rgba(103,232,249,.10);opacity:.55}.wheel-waiting-callout::after{content:"";position:absolute;right:1.15rem;top:50%;width:2.8rem;height:1px;background:linear-gradient(90deg,transparent,rgba(103,232,249,.9),transparent);transform-origin:center;animation:wheel-radar-sweep 1.2s ease-out}.wheel-waiting-callout[data-pulse="true"]{animation:wheel-waiting-pulse 1.45s ease-out}:global(.queue-stat-pulse){animation:wheel-stat-pulse 1.45s ease-out}@keyframes intake-cta-pulse{0%,100%{box-shadow:0 0 16px rgba(255,0,0,.12)}50%{box-shadow:0 0 34px rgba(255,0,0,.30)}}@keyframes wheel-waiting-pulse{0%{border-color:rgba(103,232,249,.28);box-shadow:0 0 0 rgba(103,232,249,0)}35%{border-color:rgba(103,232,249,.9);box-shadow:0 0 38px rgba(103,232,249,.28)}100%{border-color:rgba(103,232,249,.45);box-shadow:0 0 0 rgba(103,232,249,0)}}@keyframes wheel-stat-pulse{0%{filter:brightness(1);box-shadow:0 0 0 rgba(103,232,249,0)}35%{filter:brightness(1.45);box-shadow:0 0 28px rgba(103,232,249,.24)}100%{filter:brightness(1);box-shadow:0 0 0 rgba(103,232,249,0)}}@keyframes wheel-radar-sweep{0%{opacity:0;transform:rotate(-45deg)}35%{opacity:.9}100%{opacity:0;transform:rotate(140deg)}}@media (prefers-reduced-motion: reduce){.intake-cta,.wheel-waiting-callout,.wheel-waiting-callout::after,:global(.queue-stat-pulse){animation:none}}`}</style></div>;
}

function QueueStat({ label, value, helper, accent = "text-foreground", pulse = false }: { label: string; value: number | string; helper: string; accent?: string; pulse?: boolean }) {
  return <div className={`border border-border bg-background/45 p-2 ${pulse ? "queue-stat-pulse border-cyan-200/45 bg-cyan-200/5" : ""}`}><p className="text-[10px] uppercase tracking-widest text-muted">{label}</p><p className={`mt-1 text-lg font-bold ${accent}`}>{value}</p><p className="mt-1 text-[10px] leading-snug text-muted">{helper}</p></div>;
}
...
function trackStatusStyle(track: QueuePublicTrack, isLanding: boolean, isCompleted: boolean): { card: string; stamp: string | null } {
  if (isCompleted) return { card: "completed-stamp border-border opacity-85", stamp: "ALREADY PLAYED" };
  ...
  if (track.lane === "priority" || track.priorityUpgradeStatus === "manual" || track.priorityUpgradeStatus === "paid") return { card: "priority-lock border-[#ffaa00]/55", stamp: "PAYMENT CLEARED · PRIORITY SIGNAL ACTIVE" };
