...
const COPY: Record<RadioQueueEntryState["status"], { label: string; title: string; detail: string; action: string }> = {
  ...
  closed: { label: "Queue closed", title: "The next transmission is ahead.", detail: "There is no active public queue right now. Check the schedule, explore past shows, or join the conversation.", action: "" },
...
export function RadioQueueEntry() {
  ...
  return (
    <section id="queue-status" aria-labelledby="radio-queue-heading" aria-busy={radioQueueEntry.status === "loading"} className="relative mb-5 scroll-mt-24 overflow-hidden border border-accent/40 bg-surface p-5 sm:p-7">
      ...
      <div className="relative">
        ...
        {radioQueueEntry.href ? <Link href={radioQueueEntry.href} prefetch={false} className="mt-5 inline-flex min-h-12 w-full items-center justify-center gap-3 bg-accent px-5 py-3 text-center text-sm font-bold uppercase tracking-widest text-background transition hover:bg-accent-dim focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent sm:w-auto">{copy.action}<span aria-hidden="true">→</span></Link>
          : radioQueueEntry.status !== "loading" ? <button type="button" onClick={refreshQueueStatus} className="mt-5 inline-flex min-h-12 items-center border border-border-light px-5 py-3 text-sm font-bold text-foreground transition hover:border-accent focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-accent">Check queue status</button> : null}
