...
export type ArchivedBallad = { audioId: string; versionId: string; selectedAt: string; archivedAt: string; publishedAt: string | null; presentation: BalladPresentation };
export type BalladDocument = {
  ...
  published: { versionId: string; audioId: string; at: string; presentation: BalladPresentation } | null;
};
...
export type PublicBallad = { show: BalladShow; version: Pick<BalladVersion, "id" | "title" | "lyrics" | "style" | "palette" | "author">; presentation: BalladPresentation; publishedAt: string; duration: number | null; audioId: string };
...
export function publicBallad(doc: BalladDocument, show: BalladShow): PublicBallad | null {
  ...
  return { show, version: { id: version.id, title: version.title, lyrics: version.lyrics, style: version.style, palette: version.palette, author: version.author }, presentation: release.presentation, publishedAt: release.at, duration: audio.duration, audioId: audio.id };
}
...
export function publishBallad(doc: BalladDocument, now = new Date().toISOString()): BalladDocument {
  ...
  return { ...doc, published: { versionId: audio.versionId, audioId: audio.id, at: now, presentation: { ...doc.presentation } } };
}
