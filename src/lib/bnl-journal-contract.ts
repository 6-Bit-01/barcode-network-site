...
export function countBotJournalWordsInText(text: string) { return text.match(/[\p{L}\p{N}_]+/gu)?.length ?? 0; }
export function countJournalWords(entry: Pick<BNLJournalEntry, "title" | "excerpt" | "sections">) { return [entry.title, entry.excerpt, ...entry.sections.flatMap((s) => [s.heading, s.body])].reduce((total, text) => total + countBotJournalWordsInText(text), 0); }
