...
function numOpt(value: unknown): boolean { return value == null || finite(value); }
...
export function isResolvedLiveOverlayScene(value: unknown): value is ResolvedLiveOverlayScene {
  ...
  if (!hasOwn(OVERLAY_MODES, s.mode) || !hasOwn(OVERLAY_MODES, s.resolvedMode) || !str(s.reason) || !str(s.title) || typeof s.automatic !== "boolean" || typeof s.overrideActive !== "boolean" || typeof s.wheelOverlayActive !== "boolean" || !finite(s.priority) || !finite(s.wheelSpinsOwed) || !iso(s.updatedAt)) return false;
