...
function load(file, mocks = {}) {
  ...
  vm.runInNewContext(code, { module: cjsModule, exports: cjsModule.exports, require: id => mocks[id] ?? (id === "@/components/SiteAudioProvider" ? { useSiteAudio: () => ({ track: null, playlist: [], currentTime: 0, duration: 0, status: "idle", controller: {} }) } : id === "next/link" ? (({ href, children, ...props }) => React.createElement("a", { href, ...props }, children)) : id === "next/form" ? (({ children, ...props }) => React.createElement("form", props, children)) : undefined) ?? (id.startsWith("@/") ? load(`src/${id.slice(2)}${existsSync(`src/${id.slice(2)}.tsx`) ? ".tsx" : ".ts"}`, mocks) : require(id)), structuredClone, process, console, Buffer, URL, URLSearchParams, Request, Response, Date, JSON, Set }, { filename: file });
