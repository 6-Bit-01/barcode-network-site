...
assert.equal(sourceReceiver.includes("YOUTUBE_HEARTBEAT_DRIFT_THRESHOLD_SECONDS = 2.5") && sourceReceiver.includes("TIKTOK_HEARTBEAT_DRIFT_THRESHOLD_SECONDS = 2.5") && sourceReceiver.includes("PLAYER_HEARTBEAT_CORRECTION_COOLDOWN_MS = 10_000"), true, "ordinary heartbeats cannot trigger repeated subsecond provider seeks");
assert.equal(sourceReceiver.includes('const heartbeatPlaying = reason === "heartbeat" && nextSync.playbackState === "playing"') && sourceReceiver.includes("heartbeatPlaying ? PLAYER_HEARTBEAT_CORRECTION_COOLDOWN_MS : PLAYER_CORRECTION_COOLDOWN_MS"), true, "both provider players treat heartbeats as coarse recovery rather than continuous seek commands");
