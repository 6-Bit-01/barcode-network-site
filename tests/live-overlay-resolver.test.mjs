...
import { buildWheelSegments, derangedWheelCandidateOrder, detectMaterialPlaybackSeek, projectObservedPlaybackTime, resolveLiveOverlayScene, roundPlaybackDriftSeconds, serverRelativeSyncAgeSeconds, serverStampTikTokSync, serverStampYouTubeSync, youtubePresentationFromUrl, WHEEL_RIGHT_POINTER_ANGLE_DEGREES, wheelFinalRotationForSegment, wheelFinalRotationForSlice, wheelSegmentAtPointer, wheelSliceIndexAtPointer, wheelUprightLabelRotationDegrees } from "../src/lib/live-overlay-resolver.ts";
...
assert.equal(overlayApiRoute.includes("const scene = await getResolvedLiveOverlayScene()") && overlayApiRoute.indexOf("const scene = await getResolvedLiveOverlayScene()") < overlayApiRoute.indexOf("serverNow: new Date().toISOString()"), true, "public overlay API returns scene and generates serverNow after scene resolution");
...
assert.equal(receiver.includes("const serverNowMs = typeof next?.serverNow === \"string\"") && receiver.includes("const clockAnchor = Number.isFinite(serverNowMs) ? { serverNowMs, receivedAtPerformanceMs } : null") && receiver.includes("serverClockAnchorRef.current = clockAnchor") && receiver.indexOf("serverClockAnchorRef.current = clockAnchor") < receiver.indexOf("setScene((current) => nextScene ?? current)"), true, "poll validates serverNow and anchors before applying scene");
...
assert.equal((receiver.match(/<YouTubeOverlayPlayer sync=\{scene.youtube\} clockAnchorRef=\{serverClockAnchorRef\} clockAnchored=\{serverClockAnchored\} \/>/g) ?? []).length, 1, "YouTubeOverlayPlayer receives the stable clock-anchor ref plus boolean diagnostics");
assert.equal(receiver.includes('clockAnchorRef={serverClockAnchorRef}') && receiver.includes('clockAnchored={serverClockAnchored}') && receiver.includes('function TikTokOverlayPlayer({ sync, artistName, trackTitle, clockAnchorRef, clockAnchored }'), true, "TikTokOverlayPlayer receives the stable clock-anchor ref plus boolean diagnostics");
...
assert.equal(!sourceAdmin.includes("estimatedOneWayLatencySeconds"), true, "RTT helper is no longer used to alter authoritative media time");
assert.equal(sourceAdmin.includes("const publishTime = currentTimeSeconds") && !sourceAdmin.includes("const compensatedTime"), true, "YouTube playing publication uses sampled getCurrentTime without RTT addition");
assert.equal(sourceAdmin.includes('const publishTime = playbackState === "playing" ? projected : observedTimeSeconds'), true, "TikTok playing publication uses official-time projection without RTT addition");
assert.equal(sourceAdmin.includes("publishRttMsRef.current = updateRttEstimate"), true, "RTT measurement remains diagnostic-only in admin publishing");
...
assert.equal(sourceReceiver.includes("!Number.isFinite(localTimeRef.current)") && sourceReceiver.includes('sendTikTokSeekCommand(nextSync.playbackState === "playing" ? expected : nextSync.currentTimeSeconds)'), true, "First synchronization forces seek");
...
assert.equal(expectedYoutubeSource.includes("serverRelativeAgeFromAnchor") && expectedYoutubeSource.includes("sync.currentTimeSeconds + ageSeconds") && !expectedYoutubeSource.includes("Date.now()"), true, "YouTube expected time uses server-relative age and not browser Date.now");
...
assert.equal(sourceReceiver.includes("data-overlay-server-clock={clockAnchored ? \"anchored\" : \"missing\"}") && !sourceReceiver.includes("data-youtube-rtt-ms") && !sourceReceiver.includes("data-tiktok-rtt-ms"), true, "receiver exposes safe server-clock diagnostics and omits unpopulated RTT attributes");
