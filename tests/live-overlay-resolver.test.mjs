...
assert.equal(sourceAdminOverlayRoute.includes("updateLiveOverlayPlayerSync(body.sync, serverRequestReceivedAt)") && sourceLiveOverlay.includes("updateLiveOverlayPlayerSync(input: unknown, receivedAt = new Date())") && sourceLiveOverlay.includes("setLiveOverlayPlayerSync(normalizePlayerSync(input, receivedAt), receivedAt)"), true, "updatePlayerSync receives the route receipt Date for authoritative server stamping without rebuilding a snapshot");
