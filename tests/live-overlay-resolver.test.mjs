...
assert.equal(Object.prototype.hasOwnProperty.call(resolveLiveOverlayScene({ currentSession: session, nowPlaying: tiktokTrack, now: freshNow }), "tiktokPreload"), false, "TikTok preload is removed from the scene contract");
assert.equal(resolveLiveOverlayScene({ currentSession: session, nowPlaying: tiktokTrack, now: freshNow }).mode, "now_playing", "TikTok without authority remains the normal Now Playing card");
...
assert.equal(serverStampTikTokSync(freshTikTokSync, serverReceipt)?.updatedAt, serverReceipt.toISOString(), "TikTok stamping has no scheduled-start contract");
...
assert.equal(!sourceReceiver.includes("estimatedTikTokLocalTime") && sourceReceiver.includes("const mustSeek = !Number.isFinite(localTimeRef.current)"), true, "Literal TikTok rollback uses provider-observed localTimeRef instead of simulated local progression");
...
assert.equal(sourceAdmin.includes('controls: "1"') && sourceAdmin.includes('play_button: "1"') && sourceAdmin.includes('volume_control: "1"') && !sourceAdmin.includes("START TIKTOK PLAYBACK") && !sourceAdmin.includes("START SYNCHRONIZED PLAYBACK"), true, "dashboard TikTok iframe uses native controls and no custom startup blocker");
assert.equal(sourceAdmin.includes("const markDashboardTikTokReady") && sourceAdmin.includes("readyRef.current = true") && sourceAdmin.includes('statusRef.current = "ready"') && sourceAdmin.includes("clearDashboardTikTokReadyTimer()"), true, "validated dashboard TikTok events mark the native player ready");
assert.equal(adminTikTokMessageSource.indexOf('if (event.origin !== "https://www.tiktok.com") return') < adminTikTokMessageSource.indexOf('markDashboardTikTokReady()') && adminTikTokMessageSource.indexOf("if (event.source !== iframeRef.current?.contentWindow) return") < adminTikTokMessageSource.indexOf('markDashboardTikTokReady()') && adminTikTokMessageSource.indexOf('if (payload["x-tiktok-player"] !== true) return') < adminTikTokMessageSource.indexOf('markDashboardTikTokReady()'), true, "dashboard TikTok readiness is marked only after origin/source/payload validation");
assert.equal(sourceAdmin.includes('pendingPlaybackStateRef.current = state') || sourceAdmin.includes('pendingPlaybackStateRef.current = state;') || sourceAdmin.includes('pendingPlaybackStateRef.current = null'), true, "dashboard TikTok publishes from provider state/current-time evidence");
assert.equal(sourceAdmin.includes("TIKTOK_SYNC_HEARTBEAT_MS = 1_000") && sourceAdmin.includes('lastStablePlaybackStateRef.current === "playing" || lastStablePlaybackStateRef.current === "paused"'), true, "dashboard TikTok retains one-second playing/paused heartbeat");
assert.equal(!sourceAdmin.includes("firstStartRequestedRef") && !sourceAdmin.includes("firstStartCompletedRef") && !sourceAdmin.includes("startConfirmationTimerRef") && !sourceAdmin.includes("manual_required") && !sourceAdmin.includes("loadedReadyFallbackTimerRef"), true, "dashboard experimental startup state was removed");
...
assert.equal(applyTikTokSource.includes('sendTikTokVoidCommand("play")') && !applyTikTokSource.includes("duplicateRevision") && !sourceReceiver.includes("lastAppliedSyncRevisionRef"), true, "Literal TikTok rollback removes revision suppression while preserving working play commands");
assert.equal(!sourceReceiver.includes("data-tiktok-authority") && !sourceReceiver.includes("data-tiktok-sync-revision-applied"), true, "receiver-side authority/revision diagnostics from later experiments are absent");
assert.equal(!sourceReceiver.includes("function estimatedTikTokLocalTime") && !sourceReceiver.includes("localTimelineObservedAtRef") && !sourceReceiver.includes("localTimelinePlaybackStateRef"), true, "fake local timeline progression code is absent");
assert.equal(!sourceReceiver.includes("scheduleTikTokStart") && !sourceReceiver.includes("scheduledStartAt") && !sourceReceiver.includes("startToken") && !sourceReceiver.includes("startDelayMs"), true, "scheduled-start receiver code was removed");
assert.equal(!sourceReceiver.includes("TIKTOK_PRESTART_POLL_DELAY_MS") && !sourceReceiver.includes("tiktokPreload") && sourceReceiver.includes("const nextDelayMs = OVERLAY_POLL_DELAY_MS"), true, "TikTok preload and 200ms pre-start polling were removed");
assert.equal(!sourceReceiver.includes("type TikTokOverlayBootstrapStatus") && !sourceReceiver.includes("playbackConfirmedRef") && !sourceReceiver.includes("commandedPlaybackStateRef") && !sourceReceiver.includes("activeCommandFallbackTimerRef") && !sourceReceiver.includes("confirmationTimerRef"), true, "overlay bootstrap/confirmation redesign was removed");
...
assert.equal(sourceReceiver.includes('key={`${scene.tiktok.trackId ?? "trackless"}:${scene.tiktok.postId}`}') && !sourceReceiver.includes("tiktokPlayerIdentity"), true, "Component key uses track ID and post ID and preload alone cannot mount the player");
...
assert.equal(sourceReceiver.includes("const [initialAutoplay] = useState(() => sync.playbackState === \"playing\")") && sourceReceiver.includes('autoplay: initialAutoplay ? "1" : "0"') && sourceReceiver.includes('muted: "1"') && sourceReceiver.includes('play_button: "0"'), true, "checkpoint TikTok iframe restores initialAutoplay for initial playing syncs");
assert.equal(!sourceReceiver.includes('autoplay: "0"'), true, "TikTok receiver no longer hardcodes autoplay to zero");
...
assert.equal(sourceReceiver.includes("TIKTOK PLAYER DID NOT SIGNAL READY") && sourceReceiver.includes("player_event_timeout"), true, "proven TikTok player keeps the controlled trusted-event watchdog");
assert.equal(sourceReceiver.includes('if (type !== "onPlayerError") markTrustedPlayerEvent(type)') && sourceReceiver.includes('type !== "onMute"') && sourceReceiver.includes('type !== "onVolumeChange"'), true, "trusted events include state/time/mute/volume readiness");
...
assert.equal(sourceReceiver.includes('sendTikTokVoidCommand("mute")') && sourceReceiver.includes("sendTikTokSeekCommand") && sourceReceiver.includes('sendTikTokVoidCommand("play")'), true, "Working TikTok sync sends mute, seek, then play");
assert.equal(!sourceCss.includes(".live-overlay-tiktok-player--preload") && !sourceCss.includes(".live-overlay-tiktok-bootstrap-cover") && !sourceCss.includes(".live-overlay-tiktok-iframe--bootstrapping") && sourceCss.includes(".live-overlay-tiktok-iframe--hidden"), true, "experimental preload/bootstrap CSS is removed and fatal fallback hiding remains");
...
assert.equal(sourceAdmin.includes("Open Link") && sourceAdmin.includes("Copy Link") && sourceAdmin.includes("AdminTikTokPlayer"), true, "Existing PlayerDock TikTok links remain present");
assert.equal(sourceAdmin.includes('postMessage({ type, "x-tiktok-player": true }, "https://www.tiktok.com")') === false && !sourceAdmin.includes('"*"'), true, "Admin TikTok does not use wildcard postMessage targets");
