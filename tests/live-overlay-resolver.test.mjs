...
assert.equal(applyTikTokSource.includes("lastAppliedSyncRevisionRef") && applyTikTokSource.includes("duplicateRevision") && applyTikTokSource.includes("beginStartupGrace()"), true, "TikTok receiver suppresses duplicate sync revisions while preserving startup grace");
...
assert.equal(sourceReceiver.includes("type TikTokOverlayReadinessSource") && sourceReceiver.includes("const commandReadyRef = useRef(false)") && sourceReceiver.includes("iframeReadyFallbackTimerRef") && sourceReceiver.includes("TIKTOK_OVERLAY_READY_FALLBACK_MS = 1_500"), true, "overlay separates command readiness, trusted telemetry, and iframe fallback readiness");
assert.equal(sourceReceiver.includes("markCommandReady(\"iframe_fallback\")") && sourceReceiver.includes("applyLatestAuthoritativeSync()") && sourceReceiver.includes("latestSyncRef.current = sync ?? preloadSync"), true, "iframe fallback can apply a stored authoritative sync once command-ready");
...
assert.equal(sourceReceiver.includes('autoplay: initialAutoplay ? "1" : "0"') && sourceReceiver.includes("const [initialAutoplay] = useState(false)"), true, "preloaded TikTok iframe does not autoplay before authoritative start");
...
assert.equal(!sourceReceiver.includes("TIKTOK PLAYER DID NOT SIGNAL READY") && sourceReceiver.includes("player_event_timeout") && sourceReceiver.includes("telemetryStatus: \"missing\"") && sourceReceiver.includes("activateIframeFallbackReadiness()"), true, "missing TikTok telemetry becomes nonfatal diagnostics and command fallback");
...
assert.equal(sourceReceiver.includes("data-tiktok-preload-status") && sourceReceiver.includes("data-tiktok-readiness-source") && sourceReceiver.includes("data-tiktok-telemetry-status") && sourceReceiver.includes("data-tiktok-command-ready") && sourceReceiver.includes("data-tiktok-authority") && sourceReceiver.includes("data-tiktok-sync-revision-applied") && sourceReceiver.includes("data-tiktok-startup-grace"), true, "safe TikTok preload, command readiness, telemetry, authority, revision, and startup-grace diagnostics exist");
...
const tiktokPreloadCss = sourceCss.slice(sourceCss.indexOf(".live-overlay-tiktok-player--preload"), sourceCss.indexOf(".live-overlay-tiktok-player--active"));
assert.equal(tiktokPreloadCss.includes("position: fixed") && tiktokPreloadCss.includes("left: -10000px") && tiktokPreloadCss.includes("width: 360px") && tiktokPreloadCss.includes("height: 640px") && tiktokPreloadCss.includes("opacity: 0") && !tiktokPreloadCss.includes("visibility: hidden") && !tiktokPreloadCss.includes("display: none") && !tiktokPreloadCss.includes("width: 1px") && !tiktokPreloadCss.includes("height: 1px"), true, "TikTok preload uses a full-size offscreen transparent viewport instead of a hidden 1px iframe");
