...
assert.equal(sourceReceiver.includes("const projectedLocalTime = estimatedTikTokLocalTime") && sourceReceiver.includes("projectedLocalTime === null") && sourceReceiver.includes("sendTikTokSeekCommand(seekTarget)"), true, "First synchronization seeks only when projected local time is unavailable or materially wrong");
...
assert.equal(applyTikTokSource.includes("lastAppliedSyncRevisionRef") && applyTikTokSource.includes("duplicateRevision") && applyTikTokSource.includes('sendTikTokVoidCommand("play")'), true, "TikTok receiver suppresses duplicate revisions while preserving working play commands");
...
assert.equal(sourceReceiver.includes("function estimatedTikTokLocalTime") && sourceReceiver.includes("performance.now()") && sourceReceiver.includes("localTimelineObservedAtRef") && sourceReceiver.includes("localTimelinePlaybackStateRef"), true, "receiver maintains the proven monotonic local TikTok timeline");
...
assert.equal(sourceReceiver.includes('autoplay: "0"') && sourceReceiver.includes('muted: "1"') && sourceReceiver.includes('play_button: "0"'), true, "working overlay TikTok iframe uses the proven muted controlled player URL");
