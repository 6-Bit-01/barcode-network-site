...
const missingSameTrackSync = resolveLiveOverlayScene({ currentSession: session, nowPlaying: youtubeTrack, now: freshNow });
const heldMedia = stabilizeLiveOverlayMediaScene(matchingFresh, missingSameTrackSync);
assert.equal(heldMedia.scene.youtube?.videoId, "abcdefghijk", "missing same-track sync keeps the mounted YouTube player");
assert.equal(heldMedia.retainedProvider, "youtube", "retained provider is explicit for diagnostics");
const repeatedlyHeldMedia = stabilizeLiveOverlayMediaScene(heldMedia.scene, missingSameTrackSync);
assert.equal(repeatedlyHeldMedia.scene.youtube?.videoId, "abcdefghijk", "repeated heartbeat gaps cannot unmount the same-track player");
const refreshedMediaSync = stabilizeLiveOverlayMediaScene(heldMedia.scene, matchingFresh);
assert.equal(refreshedMediaSync.retainedProvider, null, "a fresh provider heartbeat resumes direct scene ownership without a remount");
const changedTrackDuringGap = resolveLiveOverlayScene({ currentSession: session, nowPlaying: { ...youtubeTrack, id: "yt2", link: "https://youtube.com/watch?v=zzzzzzzzzzz", youtubeVideoId: "zzzzzzzzzzz" }, now: freshNow });
assert.equal(stabilizeLiveOverlayMediaScene(matchingFresh, changedTrackDuringGap).scene.youtube, undefined, "a different queue track never inherits the prior player");
const overrideDuringGap = resolveLiveOverlayScene({ currentSession: session, overlayState: { systemMessageActive: true, systemMessage: "Hold" }, nowPlaying: youtubeTrack, now: freshNow });
assert.equal(stabilizeLiveOverlayMediaScene(matchingFresh, overrideDuringGap).scene.youtube, undefined, "an authoritative override replaces the provider immediately");
...
const missingSameTikTokSync = resolveLiveOverlayScene({ currentSession: session, nowPlaying: tiktokTrack, now: freshNow });
const heldTikTokMedia = stabilizeLiveOverlayMediaScene(matchingTikTok, missingSameTikTokSync);
assert.equal(heldTikTokMedia.scene.tiktok?.postId, "6718335390845095173", "missing same-track sync keeps the mounted TikTok player");
assert.equal(heldTikTokMedia.retainedProvider, "tiktok", "retained TikTok provider is explicit for diagnostics");
...
const sessionBoundPolling = readFileSync("src/lib/session-bound-polling.ts", "utf8");
...
assert.equal(receiver.includes("YOUTUBE_BEHIND_THRESHOLD_SECONDS = 0.20") && receiver.includes("YOUTUBE_AHEAD_THRESHOLD_SECONDS = 0.70") && receiver.includes("YOUTUBE_PAUSED_DRIFT_THRESHOLD_SECONDS = 0.25"), true, "YouTube receiver keeps precise thresholds for explicit seeks and state changes");
...
assert.equal(receiver.includes("requestSeq") && receiver.includes("latestAppliedSeq") && receiver.includes("AbortController") && receiver.includes("startPermanentOverlayPolling({") && receiver.includes("standbyIntervalMs: LIVE_OVERLAY_STANDBY_POLL_INTERVAL_MS"), true, "permanent overlay polling is ordered, single-flight, and wakes between sessions");
assert.equal(sessionBoundPolling.includes("if (inFlight)") && sessionBoundPolling.includes("window.setTimeout(() => { void run(); }, intervalMs)"), true, "shared session-bound polling remains single-flight and timeout-driven");
assert.equal(receiver.includes("stabilizeLiveOverlayMediaScene(displayedScene, resolvedScene)") && receiver.includes("setScene(displayScene)") && receiver.includes("setConnected(false)"), true, "Wheel overlay keeps the same-track player mounted independently of heartbeat freshness while polling failures keep the last scene");
assert.equal(overlayApiRoute.includes("const serverRequestReceivedAt = new Date()") && overlayApiRoute.includes("const scene = await getResolvedLiveOverlayScene({ allowPrivateQueueState })") && overlayApiRoute.includes("const serverNow = new Date()") && overlayApiRoute.indexOf("const serverRequestReceivedAt = new Date()") < overlayApiRoute.indexOf("const scene = await getResolvedLiveOverlayScene({ allowPrivateQueueState })") && overlayApiRoute.indexOf("const scene = await getResolvedLiveOverlayScene({ allowPrivateQueueState })") < overlayApiRoute.indexOf("const serverNow = new Date()"), true, "legacy overlay API resolves its authorized scene between request receipt and response time generation");
assert.equal(overlayApiRoute.includes("verifyStudioOverlayToken") && overlayApiRoute.includes("allowPrivateQueueState"), true, "legacy overlay API requires the Studio capability before resolving private queue state");
assert.equal(receiver.includes("headers: studioOverlayRequestHeaders()"), true, "LiveOverlayReceiver forwards its Studio capability to both live and Wheel overlay APIs");
assert.equal(liveOverlayController.includes("isAnonymousLiveOverlayQueueProjectionPublic") && liveOverlayController.includes('session?.purpose === "live_broadcast"'), true, "anonymous legacy overlay projection is limited to enabled public broadcasts");
...
assert.equal(receiver.includes("const serverRequestReceivedAtMs = typeof next?.serverRequestReceivedAt === \"string\"") && receiver.includes("const serverNowMs = typeof next?.serverNow === \"string\"") && receiver.includes("responseTransitEstimateMs: responseTransitEstimateMsRef.current ?? 0") && receiver.includes("serverClockAnchorRef.current = clockAnchor") && receiver.indexOf("serverClockAnchorRef.current = clockAnchor") < receiver.indexOf("setScene(displayScene)"), true, "poll validates server timing and anchors before applying the stabilized scene");
...
assert.equal(sourceAdminOverlayRoute.includes("updateLiveOverlayPlayerSync(body.sync, serverRequestReceivedAt, body.startDelayMs)") && sourceLiveOverlay.includes("updateLiveOverlayPlayerSync(input: unknown, receivedAt = new Date(), startDelayMs?: number)") && sourceLiveOverlay.includes("setLiveOverlayPlayerSync(normalizePlayerSync(input, receivedAt), receivedAt, startDelayMs)"), true, "updatePlayerSync receives the route receipt Date for authoritative server stamping without rebuilding a snapshot");
assert.equal(sourceAdminOverlayRoute.includes("setLiveOverlayPlayerSync(null, serverRequestReceivedAt)") && sourceAdminOverlayRoute.includes("NextResponse.json({ ok: true }"), true, "player-sync update and clear return a small acknowledgement while preserving timing headers");
...
assert.equal(sourceAdmin.includes("outboundTransitEstimateMsRef") && sourceAdmin.includes('playbackState === "playing" && startRequestId === undefined ? (outboundTransitEstimateMsRef.current ?? 0) / 1000 : 0'), true, "YouTube/TikTok playing publications use previous bounded outbound transit only while playing");
...
assert.equal(expectedTikTokSource.includes("serverRelativeAgeFromAnchor") && expectedTikTokSource.includes("ageSeconds ?? 0") && !expectedTikTokSource.includes("Date.now()"), true, "TikTok expected time uses server-relative age and missing anchor does not extrapolate");
...
assert.equal(!sourceReceiver.includes("HEARTBEAT_DRIFT_THRESHOLD_SECONDS") && !sourceReceiver.includes("HEARTBEAT_CORRECTION_COOLDOWN_MS") && sourceReceiver.includes("PLAYER_CORRECTION_COOLDOWN_MS = 1_500"), true, "provider players retain the exact known-good correction cooldown for every sync reason");
assert.equal(sourceReceiver.includes("behindThresholdSeconds: YOUTUBE_BEHIND_THRESHOLD_SECONDS") && sourceReceiver.includes("aheadThresholdSeconds: YOUTUBE_AHEAD_THRESHOLD_SECONDS") && sourceReceiver.includes("behindThresholdSeconds: TIKTOK_BEHIND_THRESHOLD_SECONDS") && sourceReceiver.includes("aheadThresholdSeconds: TIKTOK_AHEAD_THRESHOLD_SECONDS"), true, "heartbeats retain the provider-specific known-good correction thresholds");
...
assert.equal(sourceReceiver.includes("applyTikTokSync(latestSyncRef.current)"), true, "autoplay retry reads the latest clockAnchorRef.current at retry time");
...
assert.equal(applyTikTokSource.includes("[clockAnchorRef, sendTikTokSeekCommand") && !applyTikTokSource.includes("[clockAnchor,"), true, "applyTikTokSync does not depend on a changing clockAnchor value");
