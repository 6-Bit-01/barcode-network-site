...
assert.equal(sourceAdminOverlayRoute.includes("updateLiveOverlayPlayerSync(body.sync, syncReceivedAt, body.startDelayMs)") && sourceLiveOverlay.includes("updateLiveOverlayPlayerSync(input: unknown, receivedAt = new Date(), startDelayMs?: number)") && sourceLiveOverlay.includes("setLiveOverlayPlayerSync(normalizePlayerSync(input, receivedAt), receivedAt, startDelayMs)"), true, "updatePlayerSync receives the authoritative receipt Date without rebuilding a snapshot");
