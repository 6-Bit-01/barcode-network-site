...
assert.equal(sourceAdmin.includes('type === "onPlayerReady"') && sourceAdmin.includes('type === "onStateChange"') && sourceAdmin.includes('type === "onCurrentTime"'), true, "Admin handles official ready/state/current-time transitions");
...
assert.equal(adminScheduledStartSource.includes("const startPositionSeconds") && adminScheduledStartSource.includes("sendDashboardTikTokSeekCommand(startPositionSeconds)") && adminScheduledStartSource.includes("publishOverlayTikTokSync(trackSyncInput, \"playing\", startPositionSeconds"), true, "dashboard scheduled start publishes the exact shared start position");
