...
assert.equal(receiver.includes("const serverNowMs = typeof next?.serverNow === \"string\"") && receiver.includes("const clockAnchor = Number.isFinite(serverNowMs) ? { serverNowMs, receivedAtPerformanceMs } : null") && receiver.includes("serverClockAnchorRef.current = clockAnchor") && receiver.includes("setServerClockAnchor(clockAnchor)") && receiver.indexOf("serverClockAnchorRef.current = clockAnchor") < receiver.indexOf("setScene((current) => nextScene ?? current)"), true, "poll validates serverNow and anchors before applying scene");
...
assert.equal((receiver.match(/<YouTubeOverlayPlayer sync=\{scene.youtube\} clockAnchor=\{serverClockAnchor\} \/>/g) ?? []).length, 1, "both layouts use the same YouTubeOverlayPlayer with the server clock anchor");
...
assert.equal(sourceReceiver.includes("data-overlay-server-clock={clockAnchor ? \"anchored\" : \"missing\"}") && !sourceReceiver.includes("data-youtube-rtt-ms") && !sourceReceiver.includes("data-tiktok-rtt-ms"), true, "receiver exposes safe server-clock diagnostics and omits unpopulated RTT attributes");
