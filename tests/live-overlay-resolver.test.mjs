...
import { buildWheelSegments, derangedWheelCandidateOrder, detectMaterialPlaybackSeek, estimateOneWayNetworkTransitMs, expectedScheduledTikTokPlaybackTime, playbackCorrectionTarget, projectObservedPlaybackTime, resolveLiveOverlayScene, roundPlaybackDriftSeconds, serverRelativeSyncAgeSeconds, serverStampTikTokSync, serverStampYouTubeSync, shouldCorrectPlaybackDrift, TIKTOK_MAX_SYNCHRONIZED_START_DELAY_MS, TIKTOK_MIN_SYNCHRONIZED_START_DELAY_MS, TIKTOK_SYNCHRONIZED_START_DELAY_MS, updateTransitEstimateMs, youtubePresentationFromUrl, WHEEL_RIGHT_POINTER_ANGLE_DEGREES, wheelFinalRotationForSegment, wheelFinalRotationForSlice, wheelSegmentAtPointer, wheelSliceIndexAtPointer, wheelUprightLabelRotationDegrees } from "../src/lib/live-overlay-resolver.ts";
...
const scheduledStartAt = "2026-07-11T00:00:10.000Z";
const scheduledToken = "tt-start-test1";
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "playing", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse("2026-07-11T00:00:08.500Z") }), 12, "scheduled TikTok expected time does not advance before scheduledStartAt");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "playing", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse(scheduledStartAt) }), 12, "scheduled TikTok expected time equals currentTimeSeconds at scheduledStartAt");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "playing", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse("2026-07-11T00:00:10.500Z") }), 12.5, "scheduled TikTok expected time advances by elapsed time after scheduledStartAt");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "playing", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse("2026-07-11T00:00:12.000Z"), durationSeconds: 13 }), 13, "scheduled TikTok expected time clamps to duration");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "paused", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse("2026-07-11T00:00:12.000Z") }), 12, "paused scheduled TikTok sync does not advance");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "stopped", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: Date.parse("2026-07-11T00:00:12.000Z") }), 12, "stopped scheduled TikTok sync does not advance");
assert.equal(expectedScheduledTikTokPlaybackTime({ playbackState: "playing", currentTimeSeconds: 12, scheduledStartAt, startToken: scheduledToken, serverNowMs: null }), 12, "missing clock anchor does not extrapolate scheduled TikTok sync");
...
const defaultScheduledTikTok = serverStampTikTokSync(freshTikTokSync, serverReceipt, TIKTOK_SYNCHRONIZED_START_DELAY_MS);
assert.equal(typeof defaultScheduledTikTok?.scheduledStartAt, "string", "TikTok sync accepts server-created scheduledStartAt");
assert.equal(typeof defaultScheduledTikTok?.startToken, "string", "TikTok sync accepts server-created startToken");
assert.equal(serverStampTikTokSync(freshTikTokSync, serverReceipt)?.scheduledStartAt, undefined, "existing TikTok sync without scheduled start remains valid");
assert.equal(serverStampTikTokSync({ ...freshTikTokSync, scheduledStartAt: "2099-01-01T00:00:00.000Z", startToken: "client-token" }, serverReceipt)?.scheduledStartAt, undefined, "client absolute scheduledStartAt is not trusted");
assert.equal(new Date(defaultScheduledTikTok?.scheduledStartAt ?? 0).getTime(), serverReceipt.getTime() + TIKTOK_SYNCHRONIZED_START_DELAY_MS, "default scheduled start delay is based on server receipt time");
assert.equal(new Date(serverStampTikTokSync(freshTikTokSync, serverReceipt, 1)?.scheduledStartAt ?? 0).getTime(), serverReceipt.getTime() + TIKTOK_MIN_SYNCHRONIZED_START_DELAY_MS, "TikTok synchronized start delay clamps to the minimum");
assert.equal(new Date(serverStampTikTokSync(freshTikTokSync, serverReceipt, 99_000)?.scheduledStartAt ?? 0).getTime(), serverReceipt.getTime() + TIKTOK_MAX_SYNCHRONIZED_START_DELAY_MS, "TikTok synchronized start delay clamps to the maximum");
assert.equal(serverStampTikTokSync({ ...freshTikTokSync, playbackState: "paused" }, serverReceipt, TIKTOK_SYNCHRONIZED_START_DELAY_MS)?.scheduledStartAt, undefined, "paused TikTok ignores startDelayMs");
assert.equal(serverStampTikTokSync({ ...freshTikTokSync, playbackState: "stopped" }, serverReceipt, TIKTOK_SYNCHRONIZED_START_DELAY_MS)?.scheduledStartAt, undefined, "stopped TikTok ignores startDelayMs");
assert.equal(serverStampYouTubeSync({ ...freshSync, startDelayMs: TIKTOK_SYNCHRONIZED_START_DELAY_MS }, serverReceipt)?.updatedAt, serverReceipt.toISOString(), "YouTube remains valid and does not gain TikTok scheduled start fields");
...
assert.equal(receiver.includes("setScene((current) => nextScene ?? current)") && receiver.includes("setConnected(false)"), true, "overlay polling failure preserves the last known good scene while signaling hold");
...
assert.equal(receiver.includes("const serverRequestReceivedAtMs = typeof next?.serverRequestReceivedAt === \"string\"") && receiver.includes("const serverNowMs = typeof next?.serverNow === \"string\"") && receiver.includes("responseTransitEstimateMs: responseTransitEstimateMsRef.current ?? 0") && receiver.includes("serverClockAnchorRef.current = clockAnchor") && receiver.indexOf("serverClockAnchorRef.current = clockAnchor") < receiver.indexOf("setScene((current) => nextScene ?? current)"), true, "poll validates server timing and anchors before applying scene");
...
assert.equal(sourceAdminOverlayRoute.includes("setLiveOverlayState(body, serverRequestReceivedAt)") && sourceLiveOverlay.includes("setLiveOverlayState(payload: LiveOverlayPayload, receivedAt?: Date)") && sourceLiveOverlay.includes("setLiveOverlayPlayerSync(normalizePlayerSync(payload.sync, receivedAt, payload.startDelayMs), receivedAt ?? new Date(now))"), true, "updatePlayerSync receives the route receipt Date for authoritative server stamping");
...
assert.equal(sourceReceiver.includes("!Number.isFinite(localTimeRef.current)") && sourceReceiver.includes("sendTikTokSeekCommand(seekTarget)"), true, "First synchronization forces seek");
...
const expectedTikTokSource = sourceReceiver.slice(sourceReceiver.indexOf("function expectedTikTokTime"), sourceReceiver.indexOf("type TikTokTrustedEventType"));
...
assert.equal(expectedTikTokSource.includes("expectedScheduledTikTokPlaybackTime") && expectedTikTokSource.includes("hasValidSchedule") && expectedTikTokSource.includes("serverNowFromAnchor(clockAnchor)") && expectedTikTokSource.includes("serverRelativeAgeFromAnchor(sync.updatedAt") && !expectedTikTokSource.includes("Date.now()"), true, "TikTok expected time uses scheduledStartAt for scheduled starts and updatedAt for ordinary sync");
...
const applyTikTokSource = sourceReceiver.slice(sourceReceiver.indexOf("const applyTikTokSync"), sourceReceiver.indexOf("const markTrustedPlayerEvent"));
const adminScheduledStartSource = sourceAdmin.slice(sourceAdmin.indexOf("const startSynchronizedPlayback"), sourceAdmin.indexOf("useEffect(() => {\n    clearDashboardTikTokReadyTimer"));
...
assert.equal(sourceAdmin.includes('const canStartSynchronizedPlayback = iframeLoaded && status === "ready" && synchronizedStartStatus === "waiting" && !errorLabel') && sourceAdmin.includes("disabled={!canStartSynchronizedPlayback}") && !sourceAdmin.includes('disabled={status !== "ready" || !hasOfficialCurrentTime'), true, "synchronized-start button requires loaded/ready state but no longer requires current-time evidence");
assert.equal(adminScheduledStartSource.includes("const hasObservedPosition = hasObservedCurrentTimeRef.current && Number.isFinite(latestTimeRef.current) && latestTimeRef.current >= 0") && adminScheduledStartSource.includes("let startPositionSeconds = hasObservedPosition ? latestTimeRef.current : 0") && adminScheduledStartSource.includes('setStartPositionSource(hasObservedPosition ? "official" : "zero-default")'), true, "first synchronized TikTok start uses official position when present and zero-default otherwise");
assert.equal(adminScheduledStartSource.includes("Math.min(startPositionSeconds, durationRef.current)") && adminScheduledStartSource.includes("Math.max(0, startPositionSeconds)"), true, "synchronized TikTok start position remains duration/minimum clamped");
assert.equal(!adminScheduledStartSource.includes("latestTimeObservedAtRef.current === null") && !adminScheduledStartSource.includes('projectObservedPlaybackTime("playing"') && !adminScheduledStartSource.includes("outboundTransitSeconds"), true, "scheduled first-start position does not require observed-at timing, projection, or transit offsets");
...
assert.equal(adminScheduledStartSource.includes("let startPositionSeconds") && adminScheduledStartSource.includes("sendDashboardTikTokSeekCommand(startPositionSeconds)") && adminScheduledStartSource.includes("publishOverlayTikTokSync(trackSyncInput, \"playing\", startPositionSeconds"), true, "dashboard scheduled start publishes the exact shared start position");
assert.equal(!adminScheduledStartSource.includes("projectObservedPlaybackTime(\"playing\"") && !adminScheduledStartSource.includes("outboundTransitSeconds"), true, "scheduled first-start payload does not project or add transit to start position");
assert.equal(adminTikTokMessageSource.includes("if (synchronizedStartPendingRef.current)") && adminTikTokMessageSource.includes("TIKTOK_SCHEDULED_START_EARLY_TOLERANCE_MS") && adminTikTokMessageSource.includes("sendDashboardTikTokVoidCommand(\"pause\")"), true, "preparation events are suppressed and early playing is paused instead of publishing");
assert.equal(adminTikTokMessageSource.includes("lastStablePlaybackStateRef.current = \"playing\"") && adminTikTokMessageSource.includes("synchronizedStartCompletedRef.current = true") && adminTikTokMessageSource.includes("return;"), true, "confirmed scheduled playing marks local state without immediately replacing the scheduled sync");
...
assert.equal(applyTikTokSource.includes("[clockAnchorRef, scheduleTikTokStart") && !applyTikTokSource.includes("[clockAnchor,"), true, "applyTikTokSync does not depend on a changing clockAnchor value");
...
assert.equal(sourceReceiver.includes('key={`${tiktokPlayerIdentity.trackId ?? "trackless"}:${tiktokPlayerIdentity.postId}`}'), true, "Component key uses track ID and post ID");
assert.equal(sourceReceiver.includes("latestSyncRef.current = sync ?? preloadSync") && sourceReceiver.includes("useMemo(() =>") && sourceReceiver.includes("}, [initialAutoplay, preload.postId])"), true, "Heartbeats do not alter iframe src/key");
assert.equal(sourceReceiver.includes('autoplay: initialAutoplay ? "1" : "0"') && sourceReceiver.includes("const [initialAutoplay] = useState(false)"), true, "preloaded TikTok iframe does not autoplay before authoritative scheduled start");
...
assert.equal(sourceReceiver.includes("TIKTOK PLAYER DID NOT SIGNAL READY") && sourceReceiver.includes("player_event_timeout"), true, "player-event timeout has a distinct reason");
assert.equal(sourceReceiver.includes("markTrustedPlayerEvent(type)") && sourceReceiver.includes('type !== "onMute"') && sourceReceiver.includes('type !== "onVolumeChange"'), true, "trusted events include state/time/mute/volume/error readiness fallback");
...
assert.equal(sourceReceiver.includes("AUTOPLAY BLOCKED BY BROWSER") && sourceReceiver.includes("autoplay_blocked"), true, "Second 3002 shows explicit autoplay fallback");
...
assert.equal(sourceReceiver.includes("scheduleTikTokStart") && sourceReceiver.includes("scheduledStartTimerRef") && sourceReceiver.includes("activeStartTokenRef") && sourceReceiver.includes("startupGraceUntilRef"), true, "TikTok overlay scheduled-start refs and scheduler exist");
assert.equal(sourceReceiver.includes('sendTikTokVoidCommand("pause")') && sourceReceiver.includes("sendTikTokSeekCommand(nextSync.currentTimeSeconds)") && sourceReceiver.includes("scheduledStartAtMs - serverNowMs"), true, "future TikTok scheduled start pauses and seeks before scheduling one play");
assert.equal(sourceReceiver.includes('lastAppliedPlaybackStateRef.current = "playing"') && sourceReceiver.includes("scheduledStartTimerRef.current = null") && sourceReceiver.includes("beginStartupGrace()"), true, "scheduled timer marks playing before sending one Play and starting grace");
assert.equal(sourceReceiver.includes("activeStartTokenRef.current === nextSync.startToken") && sourceReceiver.includes("return true"), true, "duplicate TikTok start tokens do not reschedule startup");
assert.equal(sourceReceiver.includes("graceActive") && sourceReceiver.includes("TIKTOK_STARTUP_GRACE_SEVERE_DRIFT_SECONDS") && sourceReceiver.includes("!startupGraceCorrectionUsedRef.current") && sourceReceiver.includes("!graceActive") && sourceReceiver.includes("playbackCorrectionTarget"), true, "startup grace suppresses proportional catch-up and allows only one severe exact correction");
assert.equal(sourceReceiver.includes("data-tiktok-preload-status") && sourceReceiver.includes("data-tiktok-start-status") && sourceReceiver.includes("data-tiktok-start-delay-ms") && sourceReceiver.includes("data-tiktok-startup-grace"), true, "safe TikTok preload and scheduled-start diagnostics exist");
assert.equal(sourceReceiver.includes("generationRef.current !== generation") && sourceReceiver.includes("window.setTimeout(play, TIKTOK_DELAYED_PLAY_MS)"), true, "Delayed play checks current generation");
...
assert.equal(sourceAdmin.includes("START SYNCHRONIZED PLAYBACK") && sourceAdmin.includes("SYNCHRONIZING SIGNAL"), true, "Admin TikTok first-start blocker and pending copy exist");
assert.equal(sourceAdmin.includes("synchronizedStartCompletedRef") && sourceAdmin.includes("synchronizedStartPendingRef") && sourceAdmin.includes("activeStartTokenRef") && sourceAdmin.includes("synchronizedStartTimerRef"), true, "Admin TikTok synchronized-start refs exist");
assert.equal(sourceAdmin.includes('postMessage({ type, "x-tiktok-player": true }, "https://www.tiktok.com")') && sourceAdmin.includes('postMessage({ type: "seekTo", value: seconds, "x-tiktok-player": true }, "https://www.tiktok.com")') && !sourceAdmin.includes('"*"'), true, "Admin TikTok safe commands use exact TikTok origin and no wildcard target");
assert.equal(sourceAdmin.includes("TIKTOK_SYNCHRONIZED_START_DELAY_MS") && sourceAdmin.includes("publishOverlayTikTokSync(trackSyncInput, \"playing\"") && sourceAdmin.includes("setSynchronizedStartStatus(\"scheduled\")"), true, "Admin TikTok publishes a scheduled playing sync for first start");
assert.equal(sourceAdmin.includes("lastStablePlaybackStateRef.current === \"playing\" || lastStablePlaybackStateRef.current === \"paused\"") && sourceAdmin.includes("synchronizedStartCompletedRef.current &&"), true, "TikTok heartbeat waits for actual first playing state after synchronized start");
