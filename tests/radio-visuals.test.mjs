...
test("perceptual pilot scenes distinguish equal-loudness songs and reach calibrated full ceilings", () => {
  const snapshot = {
    visualSeed: 41_909,
    visualMode: "track",
    sessionActive: true,
  ...
  for (const sceneName of engine.RADIO_VISUAL_PERCEPTUAL_PILOT_SCENES) {
  ...
  const sceneFoundations = engine.RADIO_VISUAL_PERCEPTUAL_PILOT_SCENES.map((sceneName) => (
    engine.radioVisualPerceptualScenePlan(sceneName, darkDrives).foundation.toFixed(4)
  ));
  assert.ok(new Set(sceneFoundations).size >= 3, "pilot families must not share one generic band-weight profile");
  ...
  for (const sceneName of engine.RADIO_VISUAL_PERCEPTUAL_PILOT_SCENES) {
    ...
    const limits = engine.RADIO_VISUAL_PERCEPTUAL_PILOT_LAYER_LIMITS[sceneName];
    for (const layer of ["bass", "mid", "treble", "tapestry"]) {
    ...
    assert.ok(fullTotal > legacyTotal + 20, `${sceneName} must have materially more headroom than its sparse legacy ceiling`);
    assert.ok(fullTotal <= 70, `${sceneName} must stay inside the existing dense-family performance envelope`);
  }
...
test("broadcast FX exhaust a twelve-effect shuffle bag before repeating", () => {
  ...
  for (const seed of [17, 43120, 2166136261]) {
    ...
    for (let occurrence = 1; occurrence < sequence.length; occurrence += 1) {
      assert.notEqual(sequence[occurrence], sequence[occurrence - 1], "broadcast FX cannot repeat its neighbor");
    }
  }
});
...
test("perceptual pilot renderers consume distinct compositional controls", () => {
  const receiver = fs.readFileSync(path.join(projectRoot, "src/components/RadioVisualsReceiver.tsx"), "utf8");
  const pilots = [
    {
  ...
  for (const pilot of pilots) {
    const start = receiver.indexOf(`function ${pilot.renderer}`);
    const end = receiver.indexOf(`function ${pilot.next}`, start + 10);
    const renderer = receiver.slice(start, end);
    assert.ok(start >= 0 && end > start, `${pilot.renderer} must remain independently implemented`);
    assert.match(renderer, new RegExp(`radioVisualPerceptualScenePlan\\("${pilot.scene}", drives\\)`));
    for (const control of pilot.controls) {
      assert.match(renderer, new RegExp(`perceptual\\.${control}\\b`), `${pilot.renderer} must consume ${control} as geometry or motion`);
    }
  }
...
test("permanent receiver is a pure portrait-safe effects surface with a stable link and bounded standby polling", () => {
  ...
  const perceptualPilotRenderers = new Set([
  ...
  for (const rendererName of [
    ...
    if (perceptualPilotRenderers.has(rendererName)) {
      assert.match(renderer, /radioVisualPerceptualScenePlan\(/,
        `${rendererName} must route legacy and richer audio through the pilot's accepted semantic adapter`);
    } else {
  ...
  const expansionActivity = expansion.slice(
    expansion.indexOf("function activity"),
    expansion.indexOf("function familyAlpha"),
  );
  const expansion30Activity = expansion30.slice(
  ...
  const expandedSceneIdentityTokens = {
    ...
    drawVoxelMegacity: ["buildingCount", "windowCount", "bridgeCount", "horizonY"],
    drawLiquidChrome: ["membraneCount", "bezierCurveTo", "specularCount", "seamY"],
    drawCellularTakeover: ["colonyCount", "cellSize", "bridgeCount", "frontX"],
    drawShatteredBroadcast: ["shardCount", "polygon", "fractureCount", "points"],
  ...
  for (const [index, rendererName] of expandedRendererNames.entries()) {
    const start = expansion.indexOf(`function ${rendererName}`);
    const nextName = expandedRendererNames[index + 1];
    const end = nextName
      ? expansion.indexOf(`function ${nextName}`, start + 10)
      : expansion.indexOf("export function drawExpandedRadioVisualMusicScene", start + 10);
    const renderer = expansion.slice(start, end);
    assert.ok(start >= 0 && end > start, `${rendererName} must be independently implemented`);
    assert.match(renderer, /const a = activity\(input\)/, `${rendererName} must use the tested audio and layer-plan activity map`);
    for (const identityToken of expandedSceneIdentityTokens[rendererName]) {
      assert.match(renderer, new RegExp(`\\b${identityToken}\\b`), `${rendererName} must retain its ${identityToken} visual language`);
    }
  }
  for (const scene of engine.RADIO_VISUAL_MUSIC_SCENES.slice(10, 20)) {
    assert.match(expansion, new RegExp(`scene === ["']${scene}["']`), `${scene} must own a production Canvas dispatch branch`);
  }
  const expansion30SceneIdentityTokens = {
    drawSpectralCathedral: ["pillarCount", "archCount", "windowCount", "roseRadius"],
    drawFerrofluidField: ["magneticWellCount", "fieldLineCount", "liquidSpikeCount", "crownPoints"],
    drawOrbitalRelay: ["orbitalRingCount", "satelliteCount", "anchorCount", "relayCount"],
    drawDataLoom: ["loomBeamCount", "warpThreadCount", "weftThreadCount", "signalShuttleCount"],
    drawMonolithArray: ["monolithCount", "indexingSeamCount", "apertureCount", "citadelHeight"],
    drawPlasmaTendrils: ["plasmaRootCount", "filamentCount", "branchSparkCount", "plasmaBraidCount"],
    drawSignalBloom: ["coreRingCount", "signalPetalCount", "filamentStamenCount", "rosetteLayerCount"],
    drawVectorSwarm: ["vectorAgentCount", "swarmLeaderCount", "formationCohesion", "formationMode"],
    drawMoireEngine: ["interferenceRingCount", "phaseTickCount", "diffractionLensCount", "focusOffset"],
    drawEclipseCorona: ["coronaArcCount", "solarFlareCount", "eclipseAlignment", "flareBandAngle"],
  };
  const expansion30RendererNames = Object.keys(expansion30SceneIdentityTokens);
  for (const [index, rendererName] of expansion30RendererNames.entries()) {
    const start = expansion30.indexOf(`function ${rendererName}`);
    const nextName = expansion30RendererNames[index + 1];
    const end = nextName
      ? expansion30.indexOf(`function ${nextName}`, start + 10)
      : expansion30.indexOf("export function drawRadioVisualMusicExpansion30Scene", start + 10);
    const renderer = expansion30.slice(start, end);
    assert.ok(start >= 0 && end > start, `${rendererName} must be independently implemented`);
    assert.match(renderer, /const a = activity\(input\)/, `${rendererName} must use the tested audio and layer-plan activity map`);
    for (const identityToken of expansion30SceneIdentityTokens[rendererName]) {
      assert.match(renderer, new RegExp(`\\b${identityToken}\\b`), `${rendererName} must retain its ${identityToken} visual language`);
    }
  }
