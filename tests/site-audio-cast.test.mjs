...
function fixture({mime='audio/mpeg',status=200}={}){
  ...
  function load(file){const mod={exports:{}};vm.runInNewContext(ts.transpileModule(readFileSync(file,'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText,{module:mod,exports:mod.exports,URL,AbortSignal,Set,window:target,fetch:async(...args)=>{requests.push(args);return new Response(null,{status,headers:{'Content-Type':mime}});},require:id=>id.startsWith('@/')?load('src/'+id.slice(2)+'.ts'):require(id)});return mod.exports;}
  return {api:load('src/lib/site-audio-cast.ts'),requests,loads,commands,options:()=>options,media,player,emit,events,context};
}
