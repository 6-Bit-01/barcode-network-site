...
HEALTH_SHA256 = "584ec9bfafc98cad022e88c1fdf5b539d1befb8e1a73594bd1976c13e84e7f19"
...
def db_capture(db_path: Path, guild: int, start: datetime, end: datetime, show_date: str, limit: int, session_id: str | None = None) -> dict:
    ...
    try:
        ...
        for row in rows:
            ...
            row.update(safe_text(row.pop("content")))
        ...
        specs = [
            ...
            ("sharedBrainReceipts", "memory_governance_shared_brain_synthesis_runs", "created_at", False,
             ["created_at", "prompt_applied", "response_sent", "source_revalidation_status"]),
        ]
...
def existing_health(root: Path, db: Path, guild: int, end: datetime) -> dict:
    ...
    try:
        if hashlib.sha256(path.read_bytes()).hexdigest() != HEALTH_SHA256:
        ...
        return {"available": True, "report": module.inspect(str(db), guild, now=end),
...
def collect(root: Path, guild: int, show_date: str, limit: int = 5000, session_id: str | None = None, skip_journal: bool = False) -> dict:
    ...
    return {"schema": "barcode_after_show_evidence_v1", "generatedAt": now.isoformat(),
