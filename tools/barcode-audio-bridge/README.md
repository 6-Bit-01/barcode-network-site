...
- `CORNERLOGO1.png` and `CORNERLOGO2.png` alternate without repeating over every selected bumper and the configured Veo-marked clips, inside the commercial screen and below the TV frame. The upcoming mark starts its slow fade during the preceding clip, is fully opaque before the marked video's first frame, stays solid for that entire video, and begins fading only after it ends. Two overlapping mark layers keep consecutive marked videos covered continuously. The alternation state continues across breaks; `CORNERLOGO2.png` renders 15% smaller than `CORNERLOGO1.png`.
