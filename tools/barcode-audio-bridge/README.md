...
- `CORNERLOGO1.png` and `CORNERLOGO2.png` alternate over the bottom-right Veo mark on the configured clips, inside the commercial screen and below the TV frame. Each mark eases in and out slowly; `CORNERLOGO2.png` renders 15% smaller than `CORNERLOGO1.png`.
...
- Most START, sponsor, fake commercial/trailer, bumper, and END clips use the established 10% cover overscan beneath the clipped aperture, preventing background pinholes at any edge without crossing over the frame.
- EverSnow, Alien, CrackedEncounters, and any other near-16:9 clip detected from its actual video dimensions use a restrained 1.5% soft overscan instead. The player trades at most 8.5% of horizontal stretch for substantially more visible top-and-bottom picture; files and playback decoding are unchanged.
