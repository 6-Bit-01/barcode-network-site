...
This build also contains a fully local commercial player. Sponsor media, fake commercials/trailers, the animated background, frame, and logos stay on the show computer. The commercial player uses its own loopback listener on `127.0.0.1:43121`; the approved Show Visuals signal server remains isolated on `127.0.0.1:43120`. The website, Vercel, Redis, queue store, BNL, and payment systems do not host or process those files.
