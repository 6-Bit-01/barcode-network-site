...
internal sealed class CommercialPlayerServer : IDisposable
{
    ...
    public CommercialPlayerServer(CommercialBreakService commercials)
    {
    ...
    private async Task AcceptLoop(TcpListener listener, CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            catch (Exception error)
            {
                ...
                {
                    BridgeLog.Write("Local commercial player accept failed.", error);
                }
            }
        }
    }
        ...
        using (client)
        {
            ...
            try
            {
                ...
                if (!originAllowed)
                {
                    ...
                    BridgeLog.Write($"Rejected local commercial player request path={path} origin={rejectedOrigin}.");
            ...
            {
                BridgeLog.Write("A local commercial player request failed.", error);
            }
        }
    }
    ...
    private static bool AdminOriginAllowed(string? origin)
    {
